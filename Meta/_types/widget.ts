...
export type TAttachedProperties =
    | TAttachedInterface
    | ObjectMetaProperties<Record<string, unknown>>;

export type TAttachedStyle<ComponentPropsInterface> =
    | TAttachedStyles<ComponentPropsInterface>
    | TStyleAttributes;
...
type TPropertiesList = string[];
interface IAttachedEditor {
    [editorPath: string]: TPropertiesList;
}
...
export interface IWidgetMeta<ComponentPropsInterface extends object>
    ...
    is: MetaClass.widget | MetaClass.page;
    ...
    attachedEditors?: IAttachedEditor;
...
> extends ObjectMeta<T, ComponentPropsInterface> {
    ...
    private _cachedStyleType: ObjectMeta<TOneOfCSSProperties>;
    ...
    private _attachedEditor?: IAttachedEditor;
    ...
    constructor(
        ...
        if (Object.keys(this._styles).length) {
            this._getCachedStyleType();
        }
        this._attachedStyles = descriptor.attachedStyles ?? {};
        this._attachedEditor = descriptor.attachedEditors ?? undefined;
    ...
    getAttachedProperties(
        ...
        return this._attachedProperties[propertyName] as ObjectMetaProperties<any>;
    }
    ...
    getAttachedStyles(
        ...
        return this._attachedStyles[
            attributeName as keyof TAttachedStyles<ComponentPropsInterface>
        ] as TStyleAttributes | TAttachedStyles<ComponentPropsInterface>;
    }
    ...
    /**
     * Меняет редакторы для сгруппированных свойств
     * @param [IAttachedEditor] - редактор для группы.
     * @public
     */
    attachedEditors(attachedEditors: IAttachedEditor): this {
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore TODO поправть типы
        return this.clone({ attachedEditors });
    }

    /**
     * Возвращает редакторы для сгруппированных свойств.
     */
    getAttachedEditors(): IAttachedEditor | undefined {
        return this._attachedEditor;
    }
    ...
    toDescriptor(): IWidgetMeta<ComponentPropsInterface> {
        return {
            ...
            attachedEditors: this._attachedEditor,
            access: this._access,
    ...
    >(descriptor: IWidgetMeta<ComponentPropsInterface>, converterKey: string) {
        // @ts-ignore надо отказываться от загрузки через функции
    ...
    private _getCachedStyleType() {
        if (!this._cachedStyleType) {
            this._cachedStyleType = new ObjectMeta<TOneOfCSSProperties>({
                is: MetaClass.object,
                info: {
                    title: 'Style',
                },
                id: StyleType.getId(),
                properties: this._styles as ObjectMetaProperties<TOneOfCSSProperties>,
            });
        }
