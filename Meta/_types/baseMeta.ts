...
type TExclude<RuntimeInterface> = Exclude<RuntimeInterface, undefined>;

/**
 * Группа редакторов для свойств
 * @public
 */
export interface IComplexEditor {
    name: string;
    properties: string[];
}

/**
 * Тип описания связанного объекта
 * @public
 */
export type TRelatedObjects = string[] | IRelatedObject[];
...
export interface IRelatedObject {
    ...
    type: string;
    /**
     * Необходимые поля из связанного объекта
     */
    fields: string[];
}
...
export interface IMeta<RuntimeInterface> {
    ...
    /**
     * Группа редакторов свойств
     * @public
     * @see Meta.getComplexEditors
     */
    readonly complexEditors?: IComplexEditor[];
     ...
     */
    readonly relatedObjects?: TRelatedObjects;
...
export class Meta<RuntimeInterface extends any> {
     ...
     * Редактор типа.
     * @public
     * @see Meta.complexEditors
     * @see Meta.getComplexEditors
     */
    protected _complexEditors?: IComplexEditor[];
     ...
     */
    protected _relatedObjects: TRelatedObjects;
    ...
    constructor(descriptor: IMeta<RuntimeInterface> = { is: MetaClass.primitive }) {
        ...
        this._nullable = descriptor.nullable;
        ...
        this._complexEditors = descriptor.complexEditors ?? undefined;
    }
    ...
    toDescriptor(): IMeta<RuntimeInterface> {
        return {
            ...
            editor:
                this._editor?.loader ||
                this._editor?.component !== undefined ||
                this._editor.props !== undefined
                    ? this._editor
                    : undefined,
            designtimeEditor:
                this._designtimeEditor?.loader ||
                this._designtimeEditor?.component ||
                this._designtimeEditor.props !== undefined
                    ? this._designtimeEditor
                    : undefined,
            defaultValue: this._defaultValue,
            sampleData: this._sampleData,
            origin: this._origin,
            complexEditors: this._complexEditors,
        };
    }
    ...
    /**
     * Устанавливает набор связанных объектов для виджета.
     ...
     */
    relatedObjects(relatedObjects: TRelatedObjects): this {
        return this.clone({
            relatedObjects,
        });
    }
    ...
    /**
     * Возвращает набор связанных объектов для виджета.
     * Подробнее о relatedObjects можно прочитать {@link https://wi.sbis.ru/doc/platform/developmentapl/interface-development/controls/widget/#_2 здесь}
     */
    getRelatedObjects(): TRelatedObjects {
    ...
    editor<EditorProps extends object = IPropertyEditorProps<RuntimeInterface>>(
        ...
        if (
            ...
            logger.error(
                ...
                Для отладки ошибки необходимо поставить точку остановы в обработчик ошибки и посмотреть по стеку какой метатип строится.`
            );
        }
        if (
    ...
    /**
     * Задает состояние нулабельности для типа.
     * @see Meta.nullable
     */
    nullable(nullable: boolean): Meta<RuntimeInterface | undefined> {
        return this.clone({ nullable }) as any;
    }

    /**
     * Сообщает о состояние нулабельности для типа.
     * @see Meta.required
     * @see Meta.optional
     */
    ...
    /**
     * Меняет параметры группы редакторов свойств.
     * @public
     * @see Meta.getComplexEditors
     */
    complexEditors(complexEditors?: IComplexEditor[]): this {
        if (!complexEditors) {
            return this.clone({ complexEditors: [] });
        }
    ...
    /**
     * Возвращает группы редакторов свойств.
     * @public
     * @see Meta.complexEditors
     */
    getComplexEditors(): IComplexEditor[] | undefined {
        return this._complexEditors;
    }
