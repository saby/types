 ...
 * @private
 */
export function isVariantMeta<RuntimeInterface extends object>(item: any): item is VariantMeta<RuntimeInterface> {
...
export function extended<
    RuntimeInterface extends object,
    Attributes extends IObjectMetaAttributes<RuntimeInterface> = IObjectMetaAttributes<RuntimeInterface>
>(attributes: Attributes, extendedTitle?: IMetaInfo['extended']): Attributes {
    const result: any = {};

    Object.keys(attributes).forEach((attributeName) => {
        const attribute = attributes[attributeName];
        result[attributeName] = Meta.meta(attribute).extended(extendedTitle);
    });
    return result;
}
