...
/**
 * Определяет, что аргумент - это enum тип.
 * @param item - Всё, что угодно.
 * @private
 */
export function isEnumMeta<RuntimeInterface extends Record<string, object>>(
    item: any
): item is EnumMeta<RuntimeInterface> {
    return item instanceof EnumMeta;
}
...
export function order<
    ...
    Properties extends
        IObjectMetaProperties<RuntimeInterface> = IObjectMetaProperties<RuntimeInterface>,
>(orderId: number, properties: Properties, saveOrder?: boolean): Properties {
    const result: any = {};
    ...
    if (properties) {
        Object.keys(properties).forEach((propertyName) => {
            ...
            const property = properties[key];
            if (property) {
                const existingOrder = Meta.meta(property).getOrder();
                if (saveOrder && existingOrder !== undefined) {
                    result[propertyName] = Meta.meta(property).order(currentOrder + existingOrder);
                } else {
                    result[propertyName] = Meta.meta(property).order(currentOrder);
                }
                currentOrder++;
            } else {
