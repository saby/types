...
type TDefaultExport<ComponentPropsInterface> = { default: IComponent<ComponentPropsInterface> };
/**
 * Интерфейс загрузчика React-компонента.
 * @public
 */
export interface IComponentLoader<ComponentPropsInterface extends object = object> {
    (
        value?: unknown
    ): Promise<IComponent<ComponentPropsInterface> | TDefaultExport<ComponentPropsInterface>>;
    isSync?: boolean;
}
...
> {
     ...
     * @public
     */
    readonly loader?: IComponentLoader<ComponentPropsInterface> | string | undefined;
...
type TLoadedComponent = IComponent<unknown> | null;
        ...
        IComponentLoaderWithProps<ComponentPropsInterface, ComponentEditablePropsInterface>
{
    ...
    constructor(
        ...
        if (typeof loader === 'string') {
            if (isLoaded(loader)) {
                newLoader = () => loadSync(loader);
                newLoader.isSync = true;
            } else {
    ...
    async load(value?: unknown): Promise<IComponent<ComponentPropsInterface>> {
        ...
        return this.component as IComponent<ComponentPropsInterface>;
    }
}
...
> extends ComponentLoaderWithProps<ComponentPropsInterface, PropsInterface> {
    ...
    constructor(
        ...
        super(descriptor);
        if (typeof descriptor.loader === 'string') {
            this._moduleName = descriptor.loader;
        }
    ...
    /**
     * Возвращает путь до редактора, если он указан строкой.
     */
    getName(): string {
    ...
    get ready(): boolean {
        ...
        return (
            ...
            properties.every((meta) => {
                ...
                const originEditor = this.editorGetter(meta.getOrigin()?.meta as Meta<unknown>);
...
export class TmpMetaEditor implements IMetaEditor {
    loader: IComponentLoader | undefined;

    constructor(
        loader: string | IComponentLoader | undefined,
        public props?: object,
        ...
        if (typeof loader === 'string') {
            if (isLoaded(loader)) {
                this.loader = () => loadSync(loader);
                this.loader.isSync = true;
            } else {
    ...
    async load(): Promise<TLoadedComponent> {
        if (!this.loader) {
            return (this.component = BROKEN_EDITOR);
        }
        const loadFn = (result: IComponent<object> | TDefaultExport<object>): TLoadedComponent => {
            if (isReactComponent(result)) {
                (this as any).component = result;
            } else if (
                typeof result === 'object' &&
                isReactComponent((result as TDefaultExport<object>)?.default)
            ) {
                (this as any).component = (result as TDefaultExport<object>).default;
            } else {
                (this as any).component = BROKEN_EDITOR;
                throw new Error('Неверный компонент');
            }
            return this.component as TLoadedComponent;
        };
        if (this.loader?.isSync) {
            const result = this.loader?.();
            return loadFn(result as unknown as IComponent<object> | TDefaultExport<object>);
        }
...
export interface IMetaEditor {
    load(): Promise<IComponent<unknown> | null>;
    ready: boolean;
