...
import { ConverterUtils, GENERATOR_ID_RANDOM_DELIMITER, MetaClass } from './format';
import type { Meta, IMeta } from '../baseMeta';
import type { ArrayMeta } from '../array';
import type { VariantMeta } from '../variant';
import type { ObjectMeta } from '../object';
import type { TAttachedProperties, TAttachedStyle, WidgetMeta } from '../widget';
...
class Serializer {
    ...
    serialize(meta: TVariantMeta): TMetaJson {
        ...
        if (
            primitive.metatype &&
            [MetaClass.variant, MetaClass.enum, MetaClass.flags].includes(primitive.metatype)
        ) {
        ...
        if (primitive.metatype === MetaClass.widget) {
            ...
            if (primitive.metaattributes) {
                ...
                if (feature) {
                    primitive.metaattributes.feature = feature;
                }
                ...
                const preview = (meta as WidgetMeta<unknown>).getPreview();
                if (preview) {
                    primitive.metaattributes.preview = preview;
                }
            }
    ...
    private getGroupSerialized(descriptor: IMeta<any>): string | undefined {
        const group = descriptor.info?.group
            ? [descriptor.info.group.uid, descriptor.info.group.name]
            : undefined;

        return this.stringifyIfNeed(group);
    }
    ...
    private serializePrimitive(meta: Meta<unknown>): TMetaJsonNewSingle {
        ...
        const group: string | undefined = this.getGroupSerialized(description);
        // @ts-ignore
        ...
        const complexEditors = this.stringifyIfNeed(description.complexEditors);
        const jsonMeta: TMetaJsonNewSingle = {
            ...
            metaattributes: {
                ...
                readonly: description.info?.disabled,
                editor,
                editorProps,
                designtimeEditor,
