...
import { AbstractCollectionMeta } from '../collection';
import { EnumMeta } from '../enum';
import { FlagsMeta } from '../flags';
import { createRightMode } from './rightmode';
import { Serializer as TypesSerializer } from 'Types/serializer';
import { RecordSet } from 'Types/collection';
import { DeserializeError, deserializeInnerNew } from './deserializeInnerNew';
import { UnionMeta } from '../legacy/union';
...
type TMetaConstructor =
    ...
    | typeof PromiseMeta
    | typeof FunctionMeta
    | typeof AbstractCollectionMeta
    | typeof VariantMeta
    | typeof EnumMeta
    | typeof FlagsMeta
    | typeof UnionMeta
    | typeof PageMeta
    | typeof RemoteProcedureMeta;

const META_CLASSES: Record<MetaClass, TMetaConstructor> = {
    ...
    [MetaClass.abstractCollection]: AbstractCollectionMeta,
    [MetaClass.variant]: VariantMeta,
    [MetaClass.enum]: EnumMeta,
    [MetaClass.flags]: FlagsMeta,
    [MetaClass.union]: UnionMeta,
    [MetaClass.page]: PageMeta,
    [MetaClass.rpc]: RemoteProcedureMeta,
};
...
function deserializeInner(json: TMetaJsonSingle[], target?: TMetaJsonSingle): TVariantMeta {
    ...
    const constructor = META_CLASSES[main.is];
    if (!constructor) {
    ...
    let descriptor = {
        ...
        // @ts-ignore
        complexEditors: main.complexEditors
            ? JSON.parse(main.complexEditors, jsonParseReviver)
            : void 0,
    ...
    if (descriptor.is === MetaClass.widget) {
        ...
        (descriptor as IWidgetMeta<object>) = {
            ...
            parent: main?.parent ? main.parent : undefined,
            preview: main?.preview ? JSON.parse(main.preview) : undefined,
            styles: main?.styles
                ? JSON.parse(main.styles)
                : // @ts-ignore
                  descriptor.properties?.['.style']?.getProperties() || undefined,
