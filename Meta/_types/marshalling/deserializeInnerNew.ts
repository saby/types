...
import { createDesignTimeEditor, createEditor } from './attributes';
import { createRightMode } from './rightmode';
import { IGroup, IMeta, Meta, MetaClass } from '../baseMeta';
...
import { EnumMeta, IEnumMeta } from '../enum';
import { FlagsMeta, IFlagsMeta } from '../flags';
...
const META_CLASSES = new Map<MetaClass | string, IMetaConstruct>([
    ...
    [
        MetaClass.flags,
        {
            constructor: FlagsMeta,
            descriptorBase: { is: MetaClass.flags, id: MetaClass.flags },
        },
    ],
    [
        MetaClass.object,
...
type TReadedIMeta = { is: string; id: string } & (
    ...
    | IFlagsMeta<Record<string, object>>
...
recipe.field('defaultValue', ['MetaAttributes'], (data) => {
    if (!!data.defaultValue) {
        return JSON.parse(data.defaultValue, jsonParseReviver);
    }
...
recipe.field('sampleData', ['MetaAttributes'], (data) => {
    if (!!data.sampleData) {
        return JSON.parse(data.sampleData, jsonParseReviver);
    }
...
recipe.field('editor', ['MetaAttributes'], (data) =>
    createEditor(data.editor, data.editorProps || data.editorprops, jsonParseReviver)
);
...
recipe.field('feature', ['MetaAttributes'], (data) => {
    ...
    if (isJsonValue(data.feature)) {
        // совместимость со старыми версиями типов
        return JSON.parse(data.feature);
    }
...
function unzipLinkedDescriptor(
    ...
    if (foundedComplexType && descriptorLink !== foundedComplexType) {
        ...
        if ('editor' in foundedComplexType) {
            if (
                descriptorLink.editor?.loader === undefined &&
                !!foundedComplexType.editor?.loader
            ) {
                descriptorLink.editor = foundedComplexType.editor;
            } else if (
                descriptorLink.editor &&
                !descriptorLink.editor?.props &&
                !!foundedComplexType.editor?.props
            ) {
                // @ts-ignore
                descriptorLink.editor.props = foundedComplexType.editor?.props;
            }
        }
