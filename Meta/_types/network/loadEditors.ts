...
async function loadAttributeEditors(type: ObjectMeta<unknown, {}>): Promise<unknown> {
    // @ts-ignore TODO должно быть Promise.allSettled. Поправить в 24.1000
    return Promise.all(Object.values(type.getProperties()).map(load));
}
...
export default async function load(type: Meta<unknown>): Promise<void> {
    ...
    if ((editor as TmpMetaEditor)?.loader) {
        if (type instanceof ObjectMeta) {
            await loadAttributeEditors(type);
        }
    ...
    let attrsEditors;
    if (type instanceof ObjectMeta) {
        attrsEditors = loadAttributeEditors(type);
    }
...
export function getComponent(
    ...
    if (meta.getEditor().component) {
        return meta.getEditor().component as IComponent<any> | null;
    }
