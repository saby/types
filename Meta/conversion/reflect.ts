...
export function implementsInterface(widget: ObjectMeta, interfaceWidget: ObjectMeta) {
    const interfaceAttrs = interfaceWidget.getProperties();
    const widgetAttrs = widget.getProperties();
...
export function isTrivialConvertable(sourceMeta: ObjectMeta, targetMeta: ObjectMeta) {
    const sourceAttrs = sourceMeta.getProperties();
    ...
    // надо проверять не только отсутсвие свойств, но и наличие isValueConvertable
...
export function isInjectable(
    ...
    if (!contentAttrKey) {
        ...
        requiredTargetContentAttr = targetMeta.getProperties()[contentAttrKey];
    }
...
export function getRequiredAttrs(meta: ObjectMeta) {
    const attrs = meta.getProperties();
...
export function convertValueOfMeta(
    ...
    const sourceAttrs = sourceType.getProperties();
    const targetAttrs = targetType.getProperties();
