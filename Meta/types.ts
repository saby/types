/**
 * Библиотека для мета-описания виджетов и их свойств.
 * @library Meta/types
 * @public
 */

export {
    IComponent, // TODO Удалить
    IComponentLoader,
    IEditorProps,
    IPropertyEditorProps,
} from './_types/components';

export {
    Meta,
    MetaClass,
    isMeta,
    IEditorLoaders,
    IPropertyEditorLayoutProps,
    isBaseWidget,
    IRelatedObject,
    TRelatedObjects,
} from './_types/baseMeta';

export { ArrayMeta } from './_types/array';
export { PromiseMeta } from './_types/promise';
export { FunctionMeta } from './_types/function';
export { RemoteProcedureMeta } from './_types/remoteProcedure';
export { VariantMeta } from './_types/variant';
export { EnumMeta } from './_types/enum';
export { FlagsMeta, IFlagsMeta } from './_types/flags';
export { ObjectMeta, ObjectMetaProperties, OBJECT_TYPE_DEFAULT_VALUE } from './_types/object';
export { WidgetMeta } from './_types/widget';
export { RightMode } from './_types/marshalling/rightmode';
export { PageMeta } from './_types/page';

export { category, group, extended, isFunctionMeta, order } from './_types/meta';

export { UnionType } from './_types/legacy/union';
export * from './_types/types';
export * from './_types/textStyles';
export { default as deserialize } from './_types/marshalling/deserializer';
export { default as fetch, fetchAll } from './_types/network/fetch';
export { MetaResponse } from './_types/network/MetaResponse';
export { default as loadEditors, getComponent } from './_types/network/loadEditors';

export {
    ValueConverterOutputType,
    ValueConverterInputType,
    BROKEN_FUNC,
    ValueConverter,
    ConversionContent,
    TConversionResult,
} from './_types/valueConverter';
