...
import jsonReviver, {
    getReviverWithStorage,
    resolveInstances,
} from 'Types/_formatter/jsonReviver';
...
describe('Types/_formatter/jsonReviver', () => {
    describe('jsonReviver()', () => {
        ...
        it('should return unchanged', () => {
            assert.strictEqual(jsonReviver('a', undefined), undefined);

            assert.strictEqual(jsonReviver('a', null), null);

            assert.strictEqual(jsonReviver('a', 1), 1);

            assert.strictEqual(jsonReviver('a', 'b'), 'b');

            const arr = [];
            assert.strictEqual(jsonReviver('a', arr), arr);

            const obj = {};
            assert.strictEqual(jsonReviver('a', obj), obj);
        });
