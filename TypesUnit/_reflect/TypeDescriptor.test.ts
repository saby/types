...
describe('Types/reflect', () => {
    ...
    describe('Проверки работы с типами (добавление, получение)', () => {
        it('Добавляем и получаем тип', () => {
            ...
            assert.strictEqual(typeDescriptor.getMetadata('typeWithInfo', 'title'), type.title);
            assert.strictEqual(
                typeDescriptor.getMetadata('typeWithInfo', 'description'),
                type.description
            );
            assert.strictEqual(typeDescriptor.getMetadata('typeWithInfo', 'icon'), type.icon);
            assert.strictEqual(
        ...
        it('Получаем режим работы с зонами доступа', () => {
            ...
            assert.equal(typeDescriptor.getPermissionsMode('typeWithPermissionMode'), 1);
        });
    ...
    describe('Source', () => {
        ...
        it('Получаем путь до конструктора или функции, который создает объект этого типа', () => {
            assert.equal(typeDescriptor.getSource('typeWithSource', 'key'), 'template');
        });
        it('Получаем путь до конструктора или функции, который создает объект этого типа с наследованием', () => {
            ...
            assert.equal(typeDescriptor.getSource('typeWithSource2', 'key'), 'template');
            assert.equal(typeDescriptor.getSource('typeWithSource2', 'key2'), 'template2');
        });
        it('Получаем путь до конструктора или функции, который создает объект этого типа с множественным наследованием', () => {
            ...
            assert.equal(typeDescriptor.getSource('typeWithSource3', 'key'), 'template');
            assert.equal(typeDescriptor.getSource('typeWithSource3', 'key2'), 'template2');
        });
        it('Получаем аргументы для конструктора или функции, переданные в reference', () => {
            assert.deepEqual(typeDescriptor.getSourceArguments('typeWithSource', 'key'), {
                prop: 'prop',
            });
        });
        it('Получаем аргументы для конструктора или функции, переданные в reference с наследованием', () => {
            ...
            assert.deepEqual(typeDescriptor.getSourceArguments('typeWithSource2', 'key'), {
                prop: 'prop',
            });
            assert.deepEqual(typeDescriptor.getSourceArguments('typeWithSource2', 'key2'), {
                prop2: 'prop2',
            });
        });
        it('Получаем аргументы для конструктора или функции, переданные в reference с множественным наследованием', () => {
            ...
            assert.deepEqual(typeDescriptor.getSourceArguments('typeWithSource3', 'key'), {
                prop: 'prop',
            });
            assert.deepEqual(typeDescriptor.getSourceArguments('typeWithSource3', 'key2'), {
                prop2: 'prop2',
            });
        });
        ...
        it('Проверяем, что объекты в source корректно объединяются.', () => {
            ...
            assert.equal(typeDescriptor.getSource('typeWithSource2', 'key'), 'template');
            assert.equal(typeDescriptor.getSourceArguments('typeWithSource2', 'key'), newArgs);
        });
    });
    ...
    describe('getMetadata', () => {
        it('getMetadata', () => {
            ...
            const assertMeta = (name: string, value: unknown) => {
                assert.strictEqual(typeDescriptor.getMetadata('SimpleType', name), value);
            };
    ...
    describe('getPropertyMetadata', () => {
        it('getPropertyMetadata', () => {
            ...
            const assertPropertyMeta = (name: string, value: unknown) => {
                assert.strictEqual(
                    typeDescriptor.getPropertyMetadata('SimpleType', 'textProperty', name),
                    value
                );
            };
    ...
    describe('Properties', () => {
        it('Получаем набор свойств', () => {
            ...
            assert.deepEqual(typeDescriptor.getProperties('typeWithProperties'), {
        ...
        it('Получаем набор свойств с наследованием', () => {
            ...
            assert.deepEqual(typeDescriptor.getProperties('typeWithProperties2'), expected);
        });
        it('Получаем набор свойств', () => {
            typeDescriptor.addType({
                typeId: 'typeWithProperties',
            ...
            assert.deepEqual(typeDescriptor.getProperties('typeWithProperties3'), expected);
        });
        it('Получаем набор свойств отсортированным массивом', () => {
            ...
            assert.deepEqual(typeDescriptor.getPropertiesArray('typeWithProperties'), expected);
        });
    ...
    describe('Permissions', () => {
        it('Получаем массив зон доступа', () => {
            ...
            assert.deepEqual(typeDescriptor.getPermissions('typeWithPermissions'), ['permission']);
        });
        it('Получаем массив зон доступа с наследованием', () => {
            ...
            assert.deepEqual(typeDescriptor.getPermissions('typeWithPermissions2'), [
                'permission2',
                'permission',
            ]);
        });
        it('Получаем массив зон доступа с множественным наследованием', () => {
            ...
            assert.deepEqual(typeDescriptor.getPermissions('typeWithPermissions3'), [
                'permission3',
                'permission',
                'permission2',
            ]);
        ...
        it('Получаем массив зон доступа с наследованием, зоны доступа должны быть уникальными', () => {
            ...
            assert.deepEqual(typeDescriptor.getPermissions('typeWithPermissions2'), ['permission']);
        });
    });
});
