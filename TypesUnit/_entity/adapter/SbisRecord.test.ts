...
describe('Types/_entity/adapter/SbisRecord', () => {
    ...
    describe('getTypeName()', () => {
        ...
        it('should return default type name for record', () => {
            const data = {
                d: [100, 'foo'],
                s: [
                    {
    ...
    describe('setTypeName()', () => {
        it('should set typename on rawData', () => {
            adapter.setTypeName('TestType');

            assert.strictEqual(adapter.getTypeName(), 'TestType');

            const rawData = adapter.getData();

            assert.containsAllKeys(rawData, ['tp']);
            assert.equal(rawData.tp, 'TestType');
        });
    });
