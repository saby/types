...
describe('Types/_meta/meta', () => {
    describe('toJSON()', () => {
        ...
        it('Сериализуются sampleData', () => {
            ...
            // @ts-ignore
            assert.deepEqual(someMeta.toJSON()[0].sampleData, { d: sample });

            const importPath = 'Foo/Bar';
            someMeta = someMeta.sampleDataImport(importPath);
            // @ts-ignore
            assert.deepEqual(someMeta.toJSON()[0].sampleData, { i: importPath });
        });

        /**
         * Автогенерированные id не должны быть указаны,
         * так как никто не может редакторы для них в контексту указать.
         */
        it('Автогенерируемые id не указаны в inherits', () => {
            const ParentType = new Meta<any>({
                is: MetaClass.primitive,
                id: 'ParentType',
                info: {
