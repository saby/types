...
import MockEditor from 'TypesUnit/_meta/types/mock';
    ...
    describe('Meta', () => {
        ...
        describe('clone()', () => {
            it('копирует все данные', () => {
                ...
                expect(result.getExtended()).equal(extended);
                expect(result.getCategory()).equal('cat');
                expect(result.getIcon()).equal('i');
                expect(result.getOrder()).equal(42);
                expect(result.getTitle()).equal('Aaa');
        ...
        describe('id()', () => {
            ...
            it('обновляет значение `inherits` у нового экземпляра класса `Meta`', () => {
                ...
                expect((result as any)._inherits).deep.equal(['setId10', 'setId11']);
            });
        });
        ...
        describe('extended()', () => {
            it('можно задать `extended`', () => {
                const extended = 'test-group-uid';
                const original = new Meta({});
                expect(original.extended(extended).getExtended()).equal(extended);
            });

            it('не создаёт новый экземпляр класса `Meta`, если `extended` не меняется', () => {
                const extended = 'test-group-uid';
            ...
            it('создаёт новый экземпляр класса `Meta` с изменённым значением `extended`', () => {
                const extended = 'test-group-uid';
                const extendedTwo = 'two';
                const original = new Meta({ info: { extended } });
                const result = original.extended(extendedTwo);
                expect(original.getExtended()).equal(extended);
                expect(result).not.equal(original);
                expect(result.getExtended()).equal(extendedTwo);
            });
        });
        ...
        describe('getExtended()', () => {
            it('возвращает название группы', () => {
                ...
                expect(result.getExtended()).equal('test-group-uid');
            });
        });
        ...
        describe('devguide()', () => {
            it('не создаёт новый экземпляр класса `Meta`, если `devguide` не меняется', () => {
                const original = new Meta({ info: { devguide: 'ok' } });
                const result = original.devguide('ok');
                expect(result).equal(original);
            });

            it('создаёт новый экземпляр класса `Meta` с изменённым `devguide`', () => {
                const original = new Meta({ info: { devguide: 'one' } });
                const result = original.devguide('two');
                expect(original.getDevguide()).equal('one');
                expect(result).not.equal(original);
                expect(result.getDevguide()).equal('two');
            });
        });
        ...
        describe('editor()', () => {
            ...
            it('`editor.loader` может принимать строку', async () => {
                const original = new Meta();
                const result = original.editor('TypesUnit/_meta/types/mock');
                await result.getEditor().load();
                expect(result.getEditor().component).equal(MockEditor);
            });
        ...
        describe('sampleData()', () => {
            ...
            it('создаёт новый экземпляр класса `Meta`, если меняется значение по-умолчанию', () => {
                ...
                expect(original.getSampleData().data).equal(42);
                expect(result).not.equal(original);
                expect(result.getSampleData().data).equal(69);
            });
        });

        describe('getSampleData()', () => {
            it('возвращает `sampleData`', () => {
                const sample = { data: 42, importPath: 'Foo/Bar' };
                const original = new Meta({ sampleData: sample });
                expect(original.getSampleData()).equal(sample);
            });
        });
