...
import { Meta, MetaClass } from 'Types/_meta/meta';
    ...
    describe('Meta', () => {
        describe('constructor()', () => {
            it('использует параметры из аргумента', () => {
                ...
                expect(result.getGroup()).equal('grp');
        ...
        describe('toDescriptor()', () => {
            it('преобразует тип в мета-описание', () => {
                ...
                expect(result.info.group).equal('grp');
        ...
        describe('group()', () => {
            it('не создаёт новый экземпляр класса `Meta`, если `group` не меняется', () => {
                const original = new Meta({ info: { group: 'ok' } });
                const result = original.group('ok');
                expect(result).equal(original);
            });

            it('создаёт новый экземпляр класса `Meta` с изменённым значением `group`', () => {
                const original = new Meta({ info: { group: 'one' } });
                const result = original.group('two');
                expect(original.getGroup()).equal('one');
                expect(result).not.equal(original);
                expect(result.getGroup()).equal('two');
            });
        });

        describe('getGroup()', () => {
            it('возвращает название группы', () => {
                const result = new Meta({ info: { group: 'grp' } });
                expect(result.getGroup()).equal('grp');
            });
        });
