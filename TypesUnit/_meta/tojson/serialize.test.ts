...
import { ObjectType, ArrayType, StringType, WidgetMeta, NullType, VariantType, RightMode } from 'Types/meta';
...
describe('Types/_meta/marshaling', () => {
    ...
    describe('WidgetMeta', () => {
        ...
        it('Сериализация access', () => {
            // @ts-ignore
            assert.deepEqual(jsonData[jsonData.length - 1].rights, JSON.stringify(resultJson.rights));
        });

        it('Сериализация components', () => {
            // @ts-ignore
            assert.deepEqual(jsonData[jsonData.length - 1].rightmode, JSON.stringify(resultJson.rightmode));
        });

        it('Сериализация feature', () => {
            // @ts-ignore
            assert.deepEqual(jsonData[jsonData.length - 1].feature, JSON.stringify(resultJson.feature));
        });
    });
