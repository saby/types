    ...
    describe('ObjectComponentLoaderWithProps', () => {
        ...
        describe('load()', () => {
            ...
            it('загружает только собственный редактор, если он задан', async () => {
                ...
                expect(
                    (result as any).getAttributes().a?.getDesigntimeEditor()
                        ?.component
                ).equal(undefined);
                expect(
                    (result as any)
                        .getAttributes()
                        .b?.getAttributes()
                        ?.c.getDesigntimeEditor().component
                ).equal(undefined);
            });
            ...
            it('загружает редакторы всех атрибутов, если собственный редактор не задан', async () => {
                ...
                expect(result.component).equal(undefined);
                expect(
                    (result as any).getAttributes().a?.getEditor()?.component
                ).equal(Editor1);
                expect(
                    (result as any)
                        .getAttributes()
                        .b?.getAttributes()
                        ?.c.getEditor().component
                ).equal(Editor2);
            });
        });
    });
