/* global assert, sinon */
define(['Env/Env', 'Types/deferred', 'Types/PromiseAPIDeferred'], function (Env, defferedLib) {
               ...
               .addCallback((res) => {
                  if (res instanceof Promise) {
                     assert.fail(
                        typeof res,
                        'object',
                        'Deferred не дождался результата Promise, а вернул его как результат!'
                     );
                  }
   ...
   describe('Types/deferred:Deferred <-> Promise', function () {
      ...
      describe('.fromPromise()', function () {
         it('Value from promise have to transfering to deferred', function (done) {
            ...
            defferedLib.Deferred.fromPromise(promise).addCallbacks(
               function (res) {
                  assert.equal(res, RESPONSE_VALUE);
                  done();
               },
         ...
         it('Error from promise have to transfering to deferred', function (done) {
            ...
            defferedLib.Deferred.fromPromise(promise)
   ...
   describe('Types/deferred:Deferred Promise API', () => {
      ...
      describe('then', () => {
         it('Метод then возвращает Promise', () => {
            assert.instanceOf(defferedLib.Deferred.success().then(null), Promise);
         });
      ...
      describe('catch', () => {
         it('Метод catch возвращает Promise', () => {
            assert.instanceOf(
               defferedLib.Deferred.fail().catch(() => {
                  /*noop*/
               }),
               Promise
            );
         ...
         it('Второй метод catch не срабатывает', (done) => {
            defferedLib.Deferred.fail()
               ...
               .catch((err) => {
                  assert.fail(err, null, 'Второй метод catch не должен срабатывать');
               })
               .then(done, done);
         });
