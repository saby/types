...
describe('Types/_entity/Record', () => {
    ...
    describe('.constructor()', () => {
        ...
        it('should set typeName into adapter', () => {
            const record = new Record({
                typeName: 'TestType',
            });

            assert.strictEqual(record.getTypeName(), 'TestType');
        });
    });
    ...
    describe('.createDeclaredFormat()', () => {
        it('should generate format taken from "format" option with properties of calculated format', () => {
            ...
            assert.equal(record.getFormat().getCount(), Object.keys(recordData).length);
            assert.isTrue(record.hasDeclaredFormat());
        });
    });
    ...
    describe('.getTypeName()', () => {
        it('should return type name from adapter', () => {
    ...
    describe('.isTyped()', () => {
        it('should return true for typed record', () => {
            const rawData = getRecordSbisData();
            rawData.tp = 'User';
            const typedRecord = new Record({ rawData, adapter: 'adapter.sbis' });

            assert.isTrue(typedRecord.isTyped());
        });

        it('should return false for non-typed record', () => {
            const rawData = getRecordSbisData();
            const typedRecord = new Record({ rawData, adapter: 'adapter.sbis' });

            assert.isFalse(typedRecord.isTyped());
        });

        it('should return false for JSON-based record', () => {
