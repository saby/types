 ...
 *
 */
export type ReduceFunc<T, U, S = undefined> = (memo: S | undefined, item: T, index: U) => S;
    ...
    implements IEnumerable<T, U>
{
     ...
     * @param factory Фабрика для преобразования коллекции.
     ...
     * </pre>
     */
    reduceRight<S = T>(callback: ReduceFunc<T, U, S>, initialValue?: S): S {
        if (arguments.length < 2) {
            return this.reverse().reduce<S>(callback);
        }
        return this.reverse().reduce<S>(callback, initialValue);
    }
     ...
     * </pre>
     */
    zipObject<S1>(values: S1[]): Record<string, S1> {
     ...
     * </pre>
     */
    group<S>(callback: (item: T) => S): Grouped<S>;
    group<S>(key: string, value: string): Grouped<S>;
     ...
     * @returns число элементов, объединенных по заданному критерию
     * @example
    ...
    max(): number {
        return this.reduce((prev, current) => {
            if (prev === undefined) {
                throw new Error('prev argument of .max() function is undefined');
            }

            if (typeof current === 'number') {
                return current > prev ? current : prev;
            } else {
                throw new Error('current argument of .max() function is not a number');
            }
        });
    }
    ...
    min(): number {
        return this.reduce((prev, current) => {
            if (prev === undefined) {
                throw new Error('prev argument of .min() function is undefined');
            }

            if (typeof current === 'number') {
                return current < prev ? current : prev;
            } else {
                throw new Error('current argument of .min() function is not a number');
            }
        });
    }
     ...
     * </pre>
     */
    where(properties: Record<string, T>): Filtered<T> {
