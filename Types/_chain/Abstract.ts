    ...
    implements IEnumerable<T, U>
{
    ...
    toObject<S = IObject<T>>(): S {
        ...
        const enumerator = this.getEnumerator();
        while (enumerator.moveNext()) {
            result[enumerator.getCurrentIndex() as unknown as string] =
                enumerator.getCurrent();
        }
        return result;
    }
    ...
    count(
        by?: string | ((item: T) => string | number | boolean)
    ): Counted<T> | number {
        if (by === undefined) {
     ...
     */
    filter(
        callback: (item: T, index: U) => boolean,
        thisArg?: object
    ): Filtered<T> {
    ...
    where(properties: object): Filtered<T> {
        ...
        return this.filter((item) => {
            return keys.reduce((prev, key) => {
                return (
                    prev &&
                    object.getPropertyValue(item, key) === properties[key]
                );
            }, true);
        });
    }
