 ...
 *
 */
export type Descriptor = string | Function | null | object;

/**
 *
 */
export type ValidateFunc = <T>(props: T, propName: keyof T, componentName: string) => Error | undefined;
...
function validate(type: Descriptor): ValidateFunc {
    ...
    if (type === null) {
        return function validateNull<T>(props: T, propName: keyof T): Error | undefined {
    ...
    switch (typeName) {
        case 'string':
            return function validateTypeName<T>(props: T, propName: keyof T): Error | undefined {
                const value = props[propName];
 ...
 * </pre>
 */
function required(this: ValidateFunc): DescriptorValidator {
 ...
 *
 */
export type RequiredValidator = () => DescriptorValidator;
 ...
 * </pre>
 */
function oneOf(this: ValidateFunc, values: unknown[]): DescriptorValidator {
 ...
 *
 */
export type oneOfValidator = (values: unknown[]) => DescriptorValidator;
 ...
 * </pre>
 */
function not(this: ValidateFunc, values: unknown[]): DescriptorValidator {
    if (!(values instanceof Array)) {
        throw new TypeError('Argument values should be an instance of Array');
    }
 ...
 *
 */
export type notValidator = (values: unknown[]) => DescriptorValidator;
 ...
 * </pre>
 */
function arrayOf(this: ValidateFunc, type: Descriptor): DescriptorValidator {
    const prev: ValidateFunc = this;
 ...
 *
 */
export type arrayOfValidator = (type: Descriptor) => DescriptorValidator;

/**
 * Создает элемент цепочки со всеми доступными валидаторами.
 * @param parent Предыдущий элемент цепи.
 */
function chain(this: unknown, parent: ValidateFunc): DescriptorValidator {
    const wrapper = (...args: Parameters<ValidateFunc>) => {
 ...
 * </pre>
 * @returns undefined - значение прошло проверку, TypeError - тип значения не соответствует требуемому.
 * @param types Desirable value types
 * @public
 */
export default function descriptor(...types: Descriptor[]): DescriptorValidator {
    if (types.length === 0) {
        throw new TypeError('You should specify one type descriptor at least');
    }

    return chain(types.length > 1 ? validateComposite(...types) : validate(types[0]));
}
