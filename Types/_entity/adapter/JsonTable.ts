    ...
    implements ITable
{
    ...
    add(record: Record<string, any>, at?: number): void {
        this._touchData();
        if (at === undefined) {
    ...
    replace(record: Record<string, any>, at: number): void {
        this._touchData();
        this._checkPosition(at);
    ...
    move(source: number, target: number): void {
        ...
        const removedElement = removed.shift();

        if (removedElement !== undefined) {
            if (target === -1) {
                this._data.unshift(removedElement);
            } else {
                this._data.splice(target, 0, removedElement);
            }
        }
    }
    ...
    copy(index: number): Record<string, any> {
        this._touchData();
