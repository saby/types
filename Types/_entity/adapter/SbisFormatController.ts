...
export class RecursiveIterator {
     ...
     */
    protected _process(
        storage: Map<number, IFieldFormat[]>,
        id?: number
    ): IFieldFormat[] | undefined {
...
export default class SbisFormatController {
    ...
    getFormat(id?: number, copy?: boolean): IFieldFormat[] {
        if (id && this._cache.has(id)) {
            const cachedFormat: IFieldFormat[] = this._cache.get(id) as IFieldFormat[];
            return copy ? this._clone(cachedFormat) : cachedFormat;
        }
        ...
        return copy
            ? this._clone(result.value as IFieldFormat[])
            : (result.value as IFieldFormat[]);
    }
