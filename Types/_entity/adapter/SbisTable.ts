...
export default class SbisTable
    extends mixin<DestroyableMixin, SbisFormatMixin>(
        DestroyableMixin,
        SbisFormatMixin
    )
    implements ITable, IMetaData, ICloneable
{
    ...
    clone<T = this>(shallow?: boolean): T {
        return new SbisTable(
            shallow ? this.getData() : this._cloneData()
        ) as unknown as T;
    }
    ...
    protected _checkRowIndex(index: number, addMode?: boolean): void {
        ...
        if (!(index >= 0 && index <= max)) {
            throw new RangeError(
                `${this._moduleName}: row index ${index} is out of bounds.`
            );
        }
    }
