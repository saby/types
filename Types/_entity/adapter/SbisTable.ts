    ...
    implements ITable, IMetaData, ICloneable
{
    ...
    readonly '[Types/_entity/adapter/IMetaData]': EntityMarker = true;

    // endregion

    // region ICloneable

    readonly '[Types/_entity/ICloneable]': EntityMarker = true;
     ...
     */
    constructor(data?: ITableFormat, _injected?: boolean) {
        super(data);
    ...
    at(index: number): ITableFormat | undefined {
    ...
    move(source: number, target: number): void {
        ...
        const element = removed.shift();
        if (element) {
            if (target === -1) {
                this._data.d.unshift(element);
            } else {
                this._data.d.splice(target, 0, element);
            }
        }
    }
    ...
    protected _getMetaDataAlias(name: string): 'r' | 'p' | 'n' | void {
        switch (name) {
            case 'results':
                return 'r';
            case 'path':
