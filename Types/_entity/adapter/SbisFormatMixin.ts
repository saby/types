 ...
 */
const fieldIndicesSymbol =
    typeof Symbol === 'undefined' ? undefined : Symbol('fieldIndices');
...
export default abstract class SbisFormatMixin {
    ...
    static initMixin(instance: any, data: GenericFormat) {
        if (data) {
            if (Object.getPrototypeOf(data) !== Object.prototype) {
                throw new TypeError(
                    "Argument 'data' should be an instance of plain Object"
                );
            }
    ...
    protected _getFieldIndex(name: string): number {
        ...
        const s = this._data.s;
        let fieldIndices = fieldIndicesSymbol
            ? s[fieldIndicesSymbol]
            : this._fieldIndices;
    ...
    protected _checkFieldIndex(index: number, appendMode?: boolean): void {
        ...
        if (!(index >= 0 && index <= max)) {
            throw new RangeError(
                `${this._moduleName}: field index "${index}" is out of bounds.`
            );
        }
    }
    ...
    protected _getFieldMeta(
        ...
        switch (type) {
            case 'real':
                (meta as IUniversalFieldRealMeta).precision = (
                    info.t as IRealFieldType
                ).p;
                break;
            ...
            case 'money':
                ...
                if (typeof info.t === 'object' && 'p' in info.t) {
                    (meta as IUniversalFieldMoneyMeta).precision = (
                        info.t as IMoneyFieldType
                    ).p;
                }
                (meta as IUniversalFieldMoneyMeta).large = !!(
                    info.t as IMoneyFieldType
                ).l;
                break;
            ...
            case 'array':
                (meta as IUniversalFieldArrayMeta).kind =
                    getFieldTypeNameByInner((info.t as IArrayFieldType).t);
                Object.assign(
                    meta,
                    this._getFieldMeta(
                        index,
    ...
    protected _buildFormatDeclaration(name: string): IFieldDeclaration {
        ...
        const declaration = this._getFieldMeta(
            index,
            type
        ...
        switch (type) {
            ...
            case 'array':
                (data.t as IArrayFieldType) = {
                    n: FIELD_TYPE[type],
                    t: getFieldInnerTypeNameByOuter(
