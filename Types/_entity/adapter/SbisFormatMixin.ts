...
import { IHashMap, EntityMarker, ComplexTypeMarker, FormatNameMarker } from 'Types/declarations';
import { FormatCarrier } from './SbisFormatController';

const entryDenormalized = Symbol('denormalized');
 ...
 *
 */
export function isField(value: any): value is IFieldType {
    return value instanceof Object && !!value.n;
}

/**
 *
 */
export type IGenericMarkedFormat = IRecordMarkedFormat | ITableMarkedFormat;
 ...
 * @public
 */
export interface ITableMarkedFormat extends ITableFormat {
    [entryDenormalized]: boolean;
}

/**
 * @public
 */
export interface IRecordMarkedFormat extends ITableFormat {
    [entryDenormalized]: boolean;
}

type ITypeFieldKeys = keyof typeof FIELD_TYPE;
type IRevertedFieldTypes = {
    [key in ITypeFieldKeys]: (typeof FIELD_TYPE)[key];
};
/**
 * Инвертированный FIELD_TYPE
 */
const FIELD_TYPE_INVERTED = (Object.keys(FIELD_TYPE) as ITypeFieldKeys[]).reduce(
    (memo: IRevertedFieldTypes, key) => {
        memo[FIELD_TYPE[key] as ITypeFieldKeys] = key;
        return memo;
    },
    {} as IRevertedFieldTypes
);
...
function getFieldTypeNameByInner(innerName: ITypeFieldKeys): string {
    return FIELD_TYPE_INVERTED[innerName] || 'string';
}

function getFieldInnerTypeNameByOuter(outerName: string, fieldName: string): string {
    let alias = outerName;
    ...
    if (typeof alias === 'object' && (alias as UniversalField).type === 'array') {
        throw new TypeError(`Field type "array of array" for field ${fieldName} is not supported.`);
    }

    const result = FIELD_TYPE[(alias + '').toLowerCase() as ITypeFieldKeys];

    if (!result) {
        throw new TypeError(
            `Field type "${JSON.stringify(alias)}" for field ${fieldName} is not supported.`
        );
    }
    return result;
}
 ...
 */
export function getFormatHash(format: IFieldFormat[], formatName?: string): string {
    const formatEntriesHash = format.map((field) => {
    ...
    if (formatName) {
        formatEntriesHash.unshift(formatName);
    }

    return formatEntriesHash.join(',');
}
 ...
 */
export function denormalizeFormats(data: IGenericMarkedFormat): void {
    ...
    eachFormatEntry(data, (entry) => {
        ...
        if ((entry as IGenericMarkedFormat)[entryDenormalized]) {
            return;
        }
...
export function normalizeFormats(data: object): object {
    ...
    eachFormatEntry(dataClone, (entry) => {
        ...
        if (format !== undefined) {
            const formatName = entry.tp;
            // TODO: accelerate this by using object comparsion by link
            const formatKey = getFormatHash(format, formatName);
            if (formatStorage.has(formatKey)) {
                const formatNumber = formatStorage.get(formatKey);
                // @ts-ignore формат может быть опциональный в 6 версии протокола, если есть ссылка на формат из другого объекта в поле f
                delete entry.s;
...
export default abstract class SbisFormatMixin {
     ...
     */
    protected _fieldIndices: Map<string, number> | null;
    ...
    constructor(data: GenericFormat, _injected?: boolean) {
    ...
    static initMixin(instance: any, data?: GenericFormat) {
        if (data) {
            ...
            denormalizeFormats(data as IGenericMarkedFormat);
    ...
    /**
     * Возвращает название формата записи/записей
     */
    getTypeName(): FormatNameMarker {
        return this._data?.tp || 'record';
    }
    ...
    protected _getFieldType(index: number): string {
        ...
        return getFieldTypeNameByInner(typeName as ITypeFieldKeys);
    }
        ...
        switch (type) {
            ...
            case 'array':
                (meta as IUniversalFieldArrayMeta).kind = getFieldTypeNameByInner(
                    (info.t as IArrayFieldType).t as ITypeFieldKeys
                );
                Object.assign(
                    meta,
    ...
    protected _checkFormatColumns(
        ...
        if (isField(innerType)) {
            innerType = innerType.n;
        }
        outerType = outer.t;
        if (isField(outerType)) {
    ...
    protected _buildSType(data: IFieldFormat, format: Field): void {
        ...
        switch (type) {
            ...
            case 'flags':
                ...
                let dictHash: IHashMap<string> | undefined;
            ...
            case 'array':
                (data.t as IArrayFieldType) = {
                    n: FIELD_TYPE[type],
                    t: getFieldInnerTypeNameByOuter((format as ArrayField).getKind(), name),
                };
                break;

            default:
                data.t = FIELD_TYPE[type as ITypeFieldKeys];
        }
    }
