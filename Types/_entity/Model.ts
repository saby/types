...
interface IPropertyGetter<T, U>
    extends PropertyGetter<T, U>,
        Partial<ICompute>,
        Partial<ITrack> {}

interface IPropertySetter<T, U>
    extends PropertySetter<T, U>,
        Partial<ICompute>,
        Partial<ITrack> {}
...
class Model<T = any> extends Record<T> implements IStateful {
    ...
    protected initModel(options?: IOptions) {
    ...
    get<K extends keyof T>(name: K): T[K] {
        ...
        if (
            'def' in property &&
            !this._getRawDataAdapter().has(name as string)
        ) {
        ...
        const value = this._processCalculatedValue(
            name as string,
            preValue,
            property.get,
            true
            ...
            try {
                ...
                if (property) {
                    if (property.set) {
                        ...
                        // Calculate new value
                        value = this._processCalculatedValue(
                            key,
                            value,
                            property.set,
                            false
                        ...
                        propertiesErrors.push(
                            new ReferenceError(`Property "${key}" is read only`)
                        );
                        return;
                    }
                }
                ...
                pairs.push([
                    key,
                    value,
                    Record.prototype.get.call(this, key),
                    true,
                ]);
            } catch (err) {
    ...
    has(name: string): boolean {
        return (
            (this._$properties && this._$properties.hasOwnProperty(name)) ||
            super.has(name)
        );
    }
            ...
            if (fieldName) {
                ...
                if (index === routeLastIndex && which.data instanceof Object) {
                    Object.keys(which.data).forEach((key) => {
                        this._deleteDependencyCache(
                            curr.concat([key]).join(ROUTE_SEPARATOR)
                        );
                    });
                }
            }
    ...
    _getSerializableState(
        ...
        if (this._defaultPropertiesValues) {
            resultState._defaultPropertiesValues =
        ...
        if (!defaultPropertiesValues.hasOwnProperty(name)) {
            ...
            if (property && 'def' in property) {
                defaultPropertiesValues[name] = [
                    property.def instanceof Function
                        ? property.def.call(this)
                        : property.def,
                ];
            } else {
                defaultPropertiesValues[name] = [];
    ...
    getKey<K extends keyof T = any>(): T[K] {
        ...
        if (!keyProperty) {
            logger.info(
                this._moduleName + '::getKey(): keyProperty is not defined'
            );
            return undefined;
        }
    ...
    getKeyProperty(): string {
        if (!this._$keyProperty) {
            this._$keyProperty =
                (this._getAdapter() as IAdapter).getKeyField(
                    this._getRawData()
                ) || '';
        }
    ...
    protected _pushDependency(name: string): void {
        if (
            this._propertiesDependencyGathering &&
    ...
    static es5Constructor() {
        Record.es5Constructor.apply(this, arguments);

        Model.prototype.initModel.apply(this, arguments);
    }
