...
import { isObservableMixin, ObserveSessionMixin } from './ObserveSessionMixin';
 ...
 * - Если кэш не содержит необходимое значение, происходит его вычисление на основе {@link Types/entity:Record#getRawData сырых данных} и создается соответствующий экземпляр объекта.
...
export function isRecord(value: any): value is Record {
    return value && !!value['[Types/_entity/Record]'];
}
        ...
        ITyped
{
    ...
    get<K extends keyof DeclaredType>(name: K): DeclaredType[K] {
        this._pushSessionDependency(name);
        ...
        if (this._isSessionGathering() && isObservableMixin(value)) {
            this._createObservableDependency(value);
        }
    ...
    protected _setPairs(pairs: pairsTuple[], errors: Error[]): IHashMap<any> | null {
        ...
        this._triggerSessionDependency(changed);
