            ...
            if (value === null) {
                ...
                if (value.hasOwnProperty('_serializeMode')) {
                    switch ((value as ExtendDate).getSQLSerializationMode()) {
                        case (Date as IExtendDateConstructor)
                            .SQL_SERIALIZE_MODE_DATE:
                            return 'date';
                        case (Date as IExtendDateConstructor)
                            .SQL_SERIALIZE_MODE_TIME:
                            return 'time';
                    }
                }
                return 'datetime';
        ...
        IStateful
{
    ...
    protected get _fieldsCache(): Map<string, any> {
        return (
            this[$fieldsCache as string] ||
            (this[$fieldsCache as string] = new Map())
        );
    }
    ...
    protected get _fieldsClone(): Map<string, any> {
        return (
            this[$fieldsClone as string] ||
            (this[$fieldsClone as string] = new Map())
        );
    }
    ...
    get<K extends keyof T>(name: K): T[K] {
        ...
        if (this._isFieldValueCacheable(value)) {
            this._addChild(
                value,
                this._getRelationNameForField(name as string)
            );
                ...
                try {
                    ...
                    if (value !== oldValue) {
                        ...
                        if (
                            ...
                            getValueOf(this._getChangedFieldValue(key)) ===
                                getValueOf(value) &&
    ...
    getOriginal<K extends keyof T>(name: K): T[K] {
        if (
            this._hasChangedField(name as string) &&
            !this._statefulPrimitive(name, true)
        ) {
    ...
    relationChanged(which: any, route: string[]): any {
        ...
        switch (which.original) {
            case Record.prototype.acceptChanges:
                if (
                    fieldName &&
                    target['[Types/_entity/IStateful]'] &&
                    !target.isChanged()
                ) {
    ...
    protected _getFieldFromRelationName(name: string): string {
        name += '';
        if (
            name.substr(0, FIELD_RELATION_PREFIX.length) ===
            FIELD_RELATION_PREFIX
        ) {
    ...
    merge(source: Record): void {
        ...
        try {
            ...
            source.each((key, val) => {
                ...
                if (!this.has(key)) {
                    const fieldFormat = recordFormat.at(
                        recordFormat.getFieldIndex(key)
                    );
    ...
    _setSerializableState(state: ISerializableState): Function {
        ...
        const fromFormattableMixin =
            FormattableMixin.prototype._setSerializableState(state);
    ...
    addField(
        format: Field | IFieldDeclaration,
        at?: number,
        value?: any
    ): void {
    ...
    protected _getFieldFormat(
        name: string,
        adapter: ITable | IRecord
    ...
    acceptChanges(
        ...
        if (fields === undefined) {
            ...
            if (!(fields instanceof Array)) {
                throw new TypeError(
                    'Argument "fields" should be an instance of Array'
                );
            }
    ...
    private _statefulPrimitive(value: any, cascade: boolean): boolean {
        return (
            ...
            ((getValueType(value) !== 'record' &&
                getValueType(value) !== 'recordset') ||
                cascade)
        );
    }
    ...
    setEventRaising(enabled: boolean, analyze?: boolean): void {
        ...
        EventRaisingMixin.prototype.setEventRaising.call(
            this,
            enabled,
            analyze
        );
     ...
     * @protected
     */
    protected _getHashMap<K extends keyof T>(
        name: K | Partial<T>,
        value?: T[K]
    ): Partial<T> {
        let map = name;
    ...
    protected _haveToClone(value: any): boolean {
        return (
            this._$cloneChanged && value && value['[Types/_entity/ICloneable]']
        );
    }
    ...
    static checkOptions(options?: IOptions) {
        if (
            options &&
            options.owner &&
            !options.owner['[Types/_collection/RecordSet]']
        ) {
            throw new TypeError(
    ...
    static es5Constructor(options?: IOptions) {
        Record.checkOptions(options);
    ...
    static filter(
        record: Record,
        callback: (name: string, value: any) => boolean
    ): Record {
    ...
    protected static addFieldTo(
        ...
        if (!format) {
            let detectedFormat: IFieldDeclaration = getValueType(
                value
