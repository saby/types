 ...
 * Для корректной работы с зависимости сначала надо загрузить {@link Types/entity:Model} и {@link Types/collection:RecordSet}, а уже потом {@link Types/entity:factory}
 * @module
 * @library
 * @public
 */
...
import { IAdapter } from '../_entity/adapter';

/**
 *
 */
export type ValueType = string | Function | IProducible;
...
function formatNumber(val: number | string, decimals: number): string {
    ...
    let weNeedDecimals: number | string;
    if (parsedVal === '.') {
        parsedVal = '.0';
    }
    ...
    if (parseFloat(parsedVal) !== 0) {
        weNeedDecimals = parseFloat(
            (parsedVal + '00000000000000000000000000000000').substr(0, decimals + 1)
        ).toFixed(decimals);
    } else {
        weNeedDecimals = parseFloat(parsedVal).toFixed(decimals);
    }

    if (typeof weNeedDecimals === 'number' && weNeedDecimals === 1) {
    ...
    return (
        ((format as RealField).getPrecision
            ...
            : (format as UniversalField).meta &&
              ((format as UniversalField).meta as IUniversalFieldRealMeta).precision) || 0
    );
}
 ...
 * @param data
 */
function serializeFlags(data: Flags<unknown>): unknown[] {
...
function convertListToRecordSet(list: List<Record>): RecordSet<Record> {
    let adapter: string | IAdapter = 'Types/entity:adapter.Json';
...
export function cast<T = unknown>(
    ...
    if (typeof TypeConstructor === 'function') {
        ...
        if (TypeConstructor.prototype['[Types/_entity/IProducible]']) {
            return (TypeConstructor as IProducibleConstructor).produceInstance(value, options);
        }
 ...
 */
export function serialize<T>(value: any, options: ISerializeOptions): T | null {
    // @ts-ignore
    options = options || {};
