...
function getDaysDifference(dateA: Date, dateB: Date): number {
    const dateANoon = new Date(
        dateA.getFullYear(),
        dateA.getMonth(),
        dateA.getDate(),
        12
    );
    const dateBNoon = new Date(
        dateB.getFullYear(),
        dateB.getMonth(),
        dateB.getDate(),
        12
 ...
 * @param {Types/entity:compare.dateDifference/DateUnits.typedef} [units] Единица расчета (если не передан, используются миллисекунды)
