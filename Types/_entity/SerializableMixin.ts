 ...
 */
const isProtoSupported: boolean =
    typeof ({} as IObjectExt).__proto__ === 'object';
 ...
 */
function createModuleNameError(
    instance: object,
    critical?: boolean,
    skip?: number
): void {
...
export default class SerializableMixin<T = any> {
    ...
    protected _checkModuleName(critical: boolean, skip?: number): void {
        ...
        if (
            !this.hasOwnProperty('_moduleName') &&
     ...
     * </pre>
     */
    static fromJSON<T = SerializableMixin, K = unknown>(
        data: ISignature<K>
    ): T {
        const initializer = this.prototype._setSerializableState(data.state);
        const instance = new this(
            data.state.$options as unknown as IOptions<T>
        );
        if (initializer) {
