...
export interface ISerializableState<T = IOptions>
...
export default abstract class FormattableMixin {
    ...
    protected _getRawData(): any {
        const shouldUseAdapter =
            this._rawDataAdapter || this.hasDeclaredFormat();
    ...
    protected _getRawDataFromOption(): any {
        return typeof this._$rawData === 'function'
            ? this._$rawData()
            : this._$rawData;
    }
    ...
    protected _getAdapter(): IAdapter | IDecorator {
        if (
            ...
            FormattableMixin.prototype._getDefaultAdapter !==
                this._getDefaultAdapter
        ) {
        ...
        if (
            this._$cow &&
            !this._$adapter['[Types/_entity/adapter/IDecorator]']
        ) {
    ...
    protected _initRawDataAdapter(adapter: GenericAdapter): void {
        ...
        if (this.hasDeclaredFormat()) {
            ...
            if (adapter['[Types/_entity/adapter/IDecorator]']) {
                adapter = (adapter as IDecorator).getOriginal() as
                    | ITable
                    | IRecord;
            }
            ...
            this._getFormat().each((fieldFormat) => {
                try {
                    ...
                    logger.info(
                        ...
                        }::constructor(): can't add raw data field (${
                            err.message
                        })`
                    );
                }
    ...
    protected _resetRawDataAdapter(data?: any): void {
        if (data === undefined) {
            if (this._rawDataAdapter && typeof this._$rawData !== 'function') {
                ...
                this._$rawData = (
                    this._rawDataAdapter as ITable | IRecord
                ).getData();
            }
        } else {
            this._$rawData = data;
    ...
    protected _getRawDataFields(): string[] {
        return (
            this._rawDataFields ||
            (this._rawDataFields = (
                this._getRawDataAdapter() as ITable
            ).getFields())
        );
    }
    ...
    protected _getFormat(build?: boolean): format.Format {
        if (!this._format) {
            if (this.hasDeclaredFormat()) {
                this._format = this._$format =
                    FormattableMixin.prototype._buildFormat(
                        this._$format,
                        () => {
    ...
    protected _buildFormat(
        ...
        if (format) {
            ...
            if (formatProto === Array.prototype) {
                const factory = resolve<Function>(
                    'Types/collection:format.factory'
                );
