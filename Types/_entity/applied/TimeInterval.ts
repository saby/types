...
const hoursInDay = 24;
const intervalKeys = ['days', 'hours', 'minutes', 'seconds', 'milliseconds'] as const;
...
function fromIntervalStrToIntervalArray(intervalStr: string): (string | number)[] {
    const intervalArray: (string | number)[] = [];
...
function fromIntervalArrayToIntervalObj(intervalArray: (string | number)[]): IIntervalObject {
    const intervalObj: IIntervalObject = {};

    intervalKeys.forEach((value, index) => {
        intervalObj[value] = toNumber(String(intervalArray[index]));
    });

    return intervalObj;
}

function keyInMillisecondsConst(key: string): key is keyof typeof millisecondsConst {
    return millisecondsConst.hasOwnProperty(key);
}

function fromIntervalObjToMilliseconds(intervalObj: IIntervalObject): number {
    let milliseconds = 0;
    for (const key in millisecondsConst) {
        if (keyInMillisecondsConst(key)) {
            const val = millisecondsConst[key];
            milliseconds += val * toNumber(intervalObj[key] as unknown as string);
        }
    }
    return milliseconds;
}

function fromMillisecondsToNormIntervalObj(milliseconds: number): Required<IIntervalObject> {
    const normIntervalObj: IIntervalObject = {};
    for (const key in millisecondsConst) {
        if (keyInMillisecondsConst(key)) {
            const val = millisecondsConst[key];
    ...
    return normIntervalObj as Required<IIntervalObject>;
}

// Converts a normalized object into a normalized string:
// {days: 1, hours: 2, minutes: 3, seconds: 4, milliseconds: 5} => "P1DT2H3M4.005S"
function fromNormIntervalObjToNormIntervalStr(normIntervalObj: Required<IIntervalObject>): string {
...
export default class TimeInterval extends mixin(SerializableMixin) {
    private _normIntervalObj: IIntervalObject | undefined;
    private _normIntervalStr: string | undefined;
     ...
     */
    constructor(source?: TimeInterval | string | (string | number)[] | IIntervalObject | number) {
        super();
        if (this instanceof TimeInterval) {
            this._normIntervalObj = undefined;
            this._normIntervalStr = undefined;

            // TODO: не хочется менять публичное API, а инициализация нужна. Так что игнорим ошибку
            // @ts-ignore
            this.set(source);
        } else {
     ...
     */
    getDays(): number {
        const days = this._normIntervalObj?.days;
        if (typeof days === 'undefined') {
            throw new Error('В этом интервале не указаны дни.');
        }
        return days;
    }
     ...
     */
    getHours(): number {
        const hours = this._normIntervalObj?.hours;
        if (typeof hours === 'undefined') {
            throw new Error('В данном интервале не указаны часы.');
        }
        return hours;
    }
     ...
     */
    getMinutes(): number {
        const minutes = this._normIntervalObj?.minutes;
        if (typeof minutes === 'undefined') {
            throw new Error('В данном интервале не указаны минуты.');
        }
        return minutes;
    }
     ...
     */
    getSeconds(): number {
        const seconds = this._normIntervalObj?.seconds;
        if (typeof seconds === 'undefined') {
            throw new Error('В данном интервале не указаны секунды.');
        }
        return seconds;
    }
    /**
    ...
    getMilliseconds(): number {
        const milliseconds = this._normIntervalObj?.milliseconds;
        if (typeof milliseconds === 'undefined') {
            throw new Error('В данном интервале не указаны миллисекунды.');
        }
        return milliseconds;
    }
    ...
    getTotalHours(): number {
        const days = this._normIntervalObj?.days;
        if (typeof days === 'undefined') {
            throw new Error('В данном интервале не указаны дни.');
        }
        const hours = this._normIntervalObj?.hours;
        if (typeof hours === 'undefined') {
            throw new Error('В данном интервале не указаны часы.');
        }
        return days * hoursInDay + hours;
    }
    ...
    getTotalMinutes(): number {
        ...
        return this.getTotalHours() * minutesInHour + minutes;
    }
    ...
    getTotalSeconds(): number {
        ...
        return this.getTotalMinutes() * secondsInMinute + seconds;
    }
    ...
    getTotalMilliseconds(): number {
        ...
        return this.getTotalSeconds() * millisecondsInSecond + milliseconds;
    }
     ...
     * Остальные элементы игнорируются. Если передается число, то оно интерпретируется, как количество миллисекунд.
     */
    set(source: TimeInterval | string | (number | string)[] | IIntervalObject | number): this {
        let type;
        ...
        if (source instanceof TimeInterval) {
            ...
            source = toNumber(source as unknown as string);
            type = 'milliseconds';
        }
        ...
        switch (type) {
            ...
            // pass through
            case 'intervalArray':
                source = fromIntervalArrayToIntervalObj(source as (number | string)[]);
            // pass through
            case 'intervalObj':
                source = fromIntervalObjToMilliseconds(source as IIntervalObject);
            // pass through
            case 'milliseconds':
                this._normIntervalObj = source = fromMillisecondsToNormIntervalObj(
                    source as number
                );
                this._normIntervalStr = fromNormIntervalObjToNormIntervalStr(
                    source as Required<IIntervalObject>
                );
                break;
            case 'timeInterval':
     ...
     */
    static toString(
        source: TimeInterval | String | (string | number)[] | IIntervalObject | Number
    ): string {
        if (source !== undefined) {
            // @ts-ignore
