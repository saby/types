...
export interface IDateSignature<T = number | string | Date>
...
function normalizeArguments(
    ...
    if (withoutTimeZoneIndex > -1) {
        return [
            ...
            args.slice(
                0,
                withoutTimeZoneIndex
            ) as DateConstructorArgumentsTuple,
        ];
    }
...
export default class DateTime extends mixin(SerializableMixin) {
    ...
    constructor(...args: (number | string | Date | boolean)[]) {
        ...
        const [withoutTimeZone, normalizedArgs]: [
            boolean,
            DateConstructorArgumentsTuple
        ] = normalizeArguments(...args);
        const instance = new Date(
            ...(normalizedArgs as [string | number | Date])
        );
