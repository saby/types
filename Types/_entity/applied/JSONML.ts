...
/**
 * Интерфейс объекта с разметкой в формате JSONML
 * @public
 */
 ...
 * @example
 * Рассмотрим базовые механизмы работы с JSONML на следущем примере:
 * <pre>
 * import { IJSONML } from 'Types/entity';
 *
 * const rawJsonml: IJSONML = [
 *     'div',
 *     {
 *         width: 100,
 *         className: 'MyDiv'
 ...
 * ]
 * </pre>
 * Проверяем является ли JSONML валидным узлом:
 * <pre>
 * JSONML.isNode(rawJsonml); // true
 * </pre>
 *
 * Проверяем является ли JSONML корневым узлом:
 * <pre>
 * JSONML.isRootNode(rawJsonml); // true
 * </pre>
 *
 * Получаем атрибуты корневого узла:
 * <pre>
 * interface IRootAttributeType {
 *     className: string;
 *     width: number
 * }
 *
 * JSONML.getAttributes<IRootAttributeType>(rawJsonml); // { width: 100, className: 'MyDiv'},
 * </pre>
 * @public
 */
