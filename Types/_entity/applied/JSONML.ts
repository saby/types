...
export default abstract class JSONML {
    ...
    static isRootNode(json: IJSONML): boolean {
        return Array.isArray(json) && JSONML.isNode(json[0] as IJSONML);
    }
    ...
    static prepend(json: IJSONML, ...children: IJSONML[]): void {
        ...
        if (JSONML.hasAttributes(json)) {
            (json as (string | object)[]).splice(
                CHILDREN_INDEX_WITH_ATTRIBUTES,
                0,
                ...children
            );
        } else {
            (json as (string | object)[]).splice(
                CHILDREN_INDEX_WITHOUT_ATTRIBUTES,
                0,
                ...children
            );
        }
     ...
     * @param json
     */
    static getNodeName(json: IJSONML): string | null {
        if (!JSONML.isNode(json)) {
            return;
        }
     ...
     * @param name
     */
    static removeAttribute<T>(json: IJSONML, name: string): T {
        if (!JSONML.isNode(json) || !JSONML.hasAttributes(json)) {
            return;
        }
     ...
     */
    static setAttributes<T extends object>(
        json: IJSONML,
        attributesObject: T
    ): void {
        if (!JSONML.isNode(json)) {
