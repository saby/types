 ...
 *
 */
export type FilterFunc<T> = (item: T, index: number) => boolean;
/**
 *
 */
export type ResolveFunc<T> = (index: number) => T | undefined;
    ...
    implements IEnumerator<T, number>
{
    ...
    moveNext(): boolean {
        ...
        if (this._filter && current && !this._filter(current, this._index)) {
