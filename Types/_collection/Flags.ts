...
export default class Flags<T>
    extends mixin<
        ...
        SerializableMixin,
        CloneableMixin
    >(Dictionary, ManyToManyMixin, SerializableMixin, CloneableMixin)
    implements IFlags<T>, IProducible, IStateful
{
    ...
    protected _setChangedField(index: number, value: IValue): void {
        ...
        if (
            this.$changedFields.hasOwnProperty(index) &&
            this.$changedFields[index] === value
        ) {
    ...
    static produceInstance<T>(data?: any, options?: IProduceOptions): Flags<T> {
        return new this({
            dictionary: this.prototype._getDictionaryByFormat(
