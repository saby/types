...
export default abstract class IndexedEnumeratorMixin<T> {
     ...
     */
    protected _enumeratorIndexes: Record<string, Record<string, number[]>>;
     ...
     */
    reIndex(action: string, start: number = 0, count: number = 0): void {
        switch (action) {
     ...
     * @param property Название свойства.
     */
    protected _getIndex(property: string): Record<string, number[]> {
    ...
    protected _setToIndex(
        ...
        if (!Object.prototype.hasOwnProperty.call(index, value as string)) {
     ...
     * @param _event Дескриптор события.
     * @param action Действие, приведшее к изменению.
