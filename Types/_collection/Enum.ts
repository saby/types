 ...
 *
 *     // Создание перечисления со справочником
 *     const colors = new Enum({
 *         dictionary: ['Red', 'Green', 'Blue']
 *     });
 *
 *     // Создание перечисления со справочником и локализованным представлением справочника
 *     const status = new Enum({
 *         dictionary: ['Active', 'Inactive', 'Pending'],
 *         localeDictionary: ['Активен', 'Неактивен', 'В ожидании']
 *     });
 * </pre>
 *
 * Получение значения перечисления
 * <pre>
 *     const index = colors.get();
 *     console.log(index); // 0
 *
 *     const value = colors.getAsValue();
 *     console.log(value); // 'Red'
 *
 *     const localizedValue = status.getAsValue(true);
 *     console.log(localizedValue); // 'Активен'
 * </pre>
 *
 * Модификация перечисления
 * <pre>
 *     // Подписка на событие изменения
 *     colors.subscribe('onChange', (event, index, value) => {
 *         console.log('Новый индекс', index);
 *         console.log('Новое значение', value);
 *     });
 *
 *     // Установка значения по индексу
 *     colors.set(1); // 1:'Green'
 *
 *     // Установка значения по значению
 *     colors.setByValue('Blue'); // 2:'Blue'
 *
 *     // Отмена изменений
 *     colors.rejectChanges();
 *     console.log(colors.get()); // Вернёт исходный индекс
 *
 *     // Подтверждение изменений
 *     colors.set(0); // 1:'Red'
 *     colors.acceptChanges();
 *     console.log(colors.isChanged()); // false
 * </pre>
 * @public
 */
