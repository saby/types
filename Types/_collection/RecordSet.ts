...
export interface IOptions<TData = unknown, T = any>
...
function checkNullId<T extends Record>(value: T, keyProperty: string): void {
    if (developerMode && keyProperty) {
        if (
            ...
            value.get(keyProperty) === null
        ) {
            logger.info(
                'Types/_collection/RecordSet: Id property must not be null'
            );
    ...
    implements IObservableObject, IProducible, IStateful
{
    ...
    add(item: TExtData<T>, at?: number): T {
        const normalizedItem = this._normalizeItems(
            [item],
            RECORD_STATE.ADDED
        )[0];
        (this._getRawDataAdapter() as adapter.ITable).add(
            normalizedItem.getRawData(true),
            at
        );
    ...
    replace(item: TExtData<T>, at: number): T {
        ...
        (this._getRawDataAdapter() as adapter.ITable).replace(
            normalizedItem.getRawData(true),
            at
        );
    ...
    _setSerializableState(state: ISerializableState): Function {
        ...
        const fromFormattableMixin =
            FormattableMixin.prototype._setSerializableState(
    ...
    addField(
        format: format.Field | format.IFieldDeclaration,
        at?: number,
        value?: any
    ): void {
    ...
    getKeyProperty(): string {
        if (!this._$keyProperty) {
            this._$keyProperty = (
                this._getAdapter() as adapter.IAdapter
            ).getKeyField(this._getRawData());
        }
    ...
    getMetaData(): any {
        ...
        const metaFormat = this._$metaFormat
            ? this._buildFormat(this._$metaFormat)
            : null;
        let metaData = {};
        ...
        if (this._$metaData) {
            ...
            if (adapter['[Types/_entity/adapter/IDecorator]']) {
                adapter = (
                    adapter as adapter.IDecorator
                ).getOriginal() as adapter.ITable;
            }

            if (adapter['[Types/_entity/adapter/IMetaData]']) {
                (adapter as adapter.IMetaData)
                    ...
                    .forEach((format) => {
                        ...
                        if (metaFormat) {
                            const fieldIndex =
                                metaFormat.getFieldIndex(fieldName);
                            if (fieldIndex > -1) {
                                fieldFormat = metaFormat.at(fieldIndex);
                            }
                        }
                        ...
                        metaData[fieldName] = cast(
                            (adapter as adapter.IMetaData).getMetaData(
                                fieldName
                            ),
                            fieldFormat || format
                        );
     ...
     * @protected
     */
    protected _normalizeItem(
        item: Record,
        format: Format,
        isEqualFormat: boolean
    ): T {
        ...
        if (isEqualFormat || format.isEqual(item.getFormat(true))) {
            ...
            const normalizedAdapter = this.getAdapter().forRecord(
                null,
                this._getRawData()
            );
            const itemAdapter = item
                .getAdapter()
                .forRecord(item.getRawData(true));
    ...
    protected _checkItem(item: T): void {
        if (!item || !item['[Types/_entity/Record]']) {
            throw new TypeError(
                'Item should be an instance of Types/entity:Record'
            );
        }
    ...
    static fromJSON<T = RecordSet, K = IOptions<unknown>>(
