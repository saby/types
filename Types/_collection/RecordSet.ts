...
import { applyMixins, logger } from '../util';
import { isEqual } from '../object';
import { EntityMarker, FormatNameMarker, IHashMap, IObjectKey } from 'Types/declarations';

const DEFAULT_MODEL = 'Types/entity:Model';
const RECORD_STATE = TypesRecord.RecordState;
 ...
 */
export type TExtData<T> = T | TypesRecord | Model;

/**
 * Интерфейс объекта метаданных для {@link Types/collection:RecordSet}
 */
export interface IMetaData extends Record<string, unknown> {
    /**
     * Путь для "хлебных крошек"
     * @remark
     * Подробнее о конфигурации списков для отображения строки итогов читайте в {@link https://wi.sbis.ru/doc/platform/developmentapl/interfacedev/components/list/list-settings/list-visual-display/results/ этом разделе}
     */
    path?: RecordSet;
    /**
     * Строка итогов
     ...
     */
    more?: boolean | number | IMetaDataMore;
    /**
     * Признак "ЕстьЕщё"
     */
    total?: number;
...
/**
 * Расширенное описание признака "ЕстьЕщё"
 */
export interface IMetaDataMore {
    /**
     * Есть элементы до страницы
     */
    before: boolean;
    /**
     * Есть элементы после страницы
     */
    after: boolean;
}

/**
 * Интерфейс опций для {@link Types/collection:RecordSet}
 * @public
 */
export interface IOptions<
    DeclaredDataType extends object = any,
    T = any,
    TMetaData extends IMetaData = any,
> extends IListOptions<T>,
        IFormattableOptions {
    /**
     * Конструктор записей, порождаемых рекордсетом. По умолчанию {@link Types/entity:Model}.
     * @see {@link Types/collection:RecordSet#getModel}
     * @see {@link Types/entity:Record}
     * @see {@link Types/entity:Model}
     */
    model?: ModelConstructor<DeclaredDataType> | Function | string;
    /**
     * Название свойства записи, содержащего первичный ключ.
     * @see {@link Types/collection:RecordSet#getKeyProperty}
     */
    keyProperty?: string;
    /**
     * Метаданные
     * @depreceated используйте {@link Types/collection:IRecordSetOptions#metaData}
     */
    meta?: TMetaData;
    /**
     * Формат полей метаданных.
     * @example
     ...
     * @see {@link Types/collection:RecordSet#getMetaData}
     * @see {@link Types/collection:RecordSet#setMetaData}
     */
    metaData?: TMetaData;
}
...
/** */
function checkNullId<T extends TypesRecord>(value: T, keyProperty: string): void {
...
// эти функции должны быть расположены в своей либе, но это приводит к зацикливанию сборки
function isInstanceWithMeta(value: any): value is adapter.IMetaData {
    return value['[Types/_entity/adapter/IMetaData]'];
}

function isDecorator(value: any): value is adapter.IDecorator {
 ...
 *     <li>манипуляции с сырыми данными посредством адаптера. За реализацию аспекта отвечает примесь {@link Types/entity:FormattableMixin FormattableMixin}.</li>
 * </ul>
...
class RecordSet<
        DeclaredDataType extends object = any,
        T extends TypesRecord<DeclaredDataType> = Model<DeclaredDataType>,
        TMetaData extends IMetaData = any,
    >
    extends ObservableList<T>
    implements IObservableObject, IProducible, IStateful, ITyped
{
    readonly '[Types/_collection/RecordSet]': EntityMarker = true;
     ...
     * </pre>
     */
    protected _$model: ModelConstructor<DeclaredDataType> | Function | string;
     ...
     * </pre>
     */
    protected _$keyProperty: IObjectKey;
     ...
     * </pre>
     */
    protected _$metaData: TMetaData;
     ...
     */
    constructor(options?: IOptions<DeclaredDataType, T, TMetaData>) {
        RecordSet.checkOptions(options);
        ...
        this.initRecordSet(options);
    }

    protected initRecordSet(options?: IOptions<T>): void {
        if (options) {
    ...
    getEnumerator(state?: RecordState): Arraywise<T> {
        ...
        if (state) {
            enumerator.setFilter((record) => {
                return record instanceof TypesRecord ? record.getState() === state : true;
            });
        }
    ...
    each(
        ...
        for (let i = 0; i < length; i++) {
            ...
            let isMatching = true;
            if (state) {
                isMatching = record instanceof TypesRecord ? record.getState() === state : true;
            }
            if (isMatching) {
                // @ts-ignore
     ...
     * @see {@link Types/collection:ObservableList#add}
     * @remark
     ...
     *         <ul>
     *             <li>{@link Types/entity:FormattableMixin#hasDeclaredFormat} если формат задан явно, то запись принудительно приводится к формату рекордсета</li>
     ...
     */
    at(index: number): T {
        return this._getRecord(index) as T;
    }

    /**
     * Удаляет запись из RecordSet. Записи, следующие за удаленным, смещаются вверх.
     * @param item Удаляемая запись
     * @return Если true - запись удалена, если false - такой записи нет в RecordSet.
     * @example
     * Удалим запись #2 из RecordSet:
     * <pre>
     *     import { RecordSet } from 'Types/collection';
     *
     *     const users = new RecordSet({
     ...
     *     });
     *
     *     const secondRecord = users.at(1);
     *     secondRecord.get('name'); //'Emmett Brown'
     *
     *     users.remove(secondRecord); //true
     *     users.at(1).get('name'); //'George McFly'
     * </pre>
     */
    ...
    /**
     * Удаляет запись в указанной позиции. Записи, следующие за удаленным, смещаются вверх.
     * При недопустимом index генерируется исключение.
     * @param index Позиция элемента
     * @return Удаленная запись
     * @example
     * Удалим первую запись RecordSet:
     * <pre>
     *     import { RecordSet } from 'Types/collection';
     *
     *     const users = new RecordSet({
     ...
     *     });
     *
     *     users.at(0).get('name'); //'Marty McFly'
     *     users.removeAt(0).get('name'); //'Marty McFly'
     *     users.at(0).get('name'); //'Emmett Brown'
     * </pre>
     */
     ...
     * @return Добавленная запись
     * @see {@link Types/collection:ObservableList#replace}
     * @example
     ...
     * @see {@link Types/collection:ObservableList#assign}
     * @example
    ...
    assign(items: IEnumerable<TExtData<T>> | TExtData<T>[]): T[] {
        ...
        items = this._itemsToArray(items as IEnumerable<any>);
     ...
     * @return Добавленные записи
     * @see {@link Types/collection:ObservableList#append}
     * @example
     ...
     * @param items Коллекция с записями для добавления
     * @return Добавленные записи
     * @see {@link Types/collection:ObservableList#prepend}
     * @example
     * Добавим записи из одного рекордсета в другой:
     * <pre>
     *     import {RecordSet} from 'Types/collection';
    ...
    _setSerializableState(state: ISerializableState): Function {
        ...
        return function (this: RecordSet): void {
            fromSuper.call(this);
    ...
    addField(format: format.Field | format.IFieldDeclaration, at?: number, value?: any): void {
        ...
        this._parentChanged(TypesRecord.prototype.addField);

        if (value !== undefined) {
            const name = format.getName();
            this.each((record: TypesRecord) => {
                if (record instanceof TypesRecord) {
    ...
    removeField(name: string): void {
        ...
        this._nextVersion();
        this._parentChanged(TypesRecord.prototype.removeField);
    }

    removeFieldAt(at: number): void {
        FormattableMixin.prototype.removeFieldAt.call(this, at);
        this._nextVersion();
        this._parentChanged(TypesRecord.prototype.removeFieldAt);
    }
     ...
     * @param keepFormatСохранить формат
     * @protected
     */
    ...
    static produceInstance<T extends TypesRecord = Model>(
        data: any,
        options?: IOptions<T>
    ): RecordSet<T> {
     ...
     * @param shallowСоздать поверхностную копию (агрегированные объекты не клонируются). Использовать поверхностные копии можно только для чтения, т.к. изменения в них будут отражаться и на оригинале.
     * @return
     * @example
    ...
    clone<U = this>(shallow?: boolean): U {
        const clone = super.clone<RecordSet<DeclaredDataType, T>>(shallow);
        if (shallow) {
     ...
     * </pre>
     */
    getModel(): ModelConstructor<DeclaredDataType> | Function | string {
        return this._$model;
    }
     ...
     * </ul>
     * @param spreadРаспространять изменения по иерархии родителей (будут вызваны acceptChanges всех владельцев).
     * @param cascadeРаспространять изменения рекурсивно по вложенным элементам. Если параметр задан, будут вызваны acceptChanges рекурсивно у всех вложенных элементов.
     * @example
     ...
     * </ul>
     * @param spreadРаспространять изменения по иерархии родителей (будут вызваны rejectChanges всех владельцев).
     * @param cascadeРаспространять изменения рекурсивно по вложенным элементам. Если параметр задан, будут вызваны rejectChanges рекурсивно у всех вложенных элементов.
     * @example
     ...
     * </pre>
     */
    getKeyProperty(): IObjectKey {
        if (!this._$keyProperty) {
            const keyProperty = (this._getAdapter() as adapter.IAdapter).getKeyField(
                this._getRawData()
            ) as string;
     ...
     * Подробнее о метаданных смотрите в описании опции {@link Types/collection:IRecordSetOptions#metaData}.
     * @see {@link setMetaData}
     * @returns Метаданные RecordSet'a
     */
    getMetaData(): TMetaData {
        ...
        this._metaData = metaData as TMetaData;
        return this._metaData;
    }
     ...
     * @param extend Расширять метаданные в сырых данных. По умолчанию обновляются только те метаданные, которые уже есть в сырых данных.
     * @see {@link getMetaData}
     */
    setMetaData(meta: TMetaData, extend: boolean = false): void {
    ...
    merge(recordSet: RecordSet, options?: IMergeOptions): void {
        ...
        const existsIdMap: Record<IObjectKey, number> = {};
        const newIdMap: Record<number, boolean> = {};
        const toAdd: TypesRecord[] = [];
        const toReplace: [TypesRecord, number][] = [];
        const toInject: [TypesRecord, number][] = [];
        let record: TypesRecord;
        let id;
        let index;

        this.each((record: TypesRecord, index) => {
            if (record instanceof TypesRecord) {
                existsIdMap[record.get<IObjectKey>(keyProperty)] = index;
            }
        });
        ...
        if (options.remove) {
            ...
            this.each((record: TypesRecord, index) => {
                if (
                    record instanceof TypesRecord &&
                    !newIdMap.hasOwnProperty(record.get(keyProperty))
                ) {
                    toRemove.push(index);
                }
            });
    ...
    getTypeName(): FormatNameMarker {
        return (this._getRawDataAdapter() as adapter.ITable).getTypeName();
    }
     ...
     * @protected
     */
    protected _normalizeItem(item: TypesRecord, format: Format, isEqualFormat?: boolean): T {
     ...
     * @protected
     */
    protected _checkItem(item: TypesRecord<any>): void {
     ...
     * @protected
     */
    protected _buildRecord(rawData: any, instanceState: IHashMap<any> | null): T {
     ...
     * @protected
     */
    protected _getRecord(at: number): T | undefined {
        ...
        if (this._isSessionGathering() && isObservableMixin(record)) {
            this._createObservableDependency(record);
        }
    ...
    toJSON(): string;
    toJSON(): ISerializableSignature<IOptions<T>> | string {
        // @ts-ignore
        return super.toJSON();
    }

    static fromJSON<T = RecordSet, K = IOptions<TypesRecord, T>>(
    ...
    static checkOptions<T>(options?: IOptions<TypesRecord, T>): void {
        if (options) {
            if ('items' in options) {
                logger.stack(
    ...
    static es5Constructor<T>(options?: IOptions<TypesRecord, T>): void {
        RecordSet.checkOptions(options);

        ObservableList.es5Constructor.call(this, options);

        RecordSet.prototype.initRecordSet.call(this, options);
    }
     ...
     */
    static patch(items: RecordSet, names?: string[]): TypesRecord {
        names = names || ['changed', 'added', 'removed'];

        const filter = (state: RecordState) => {
