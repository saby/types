    ...
    implements IObservableObject, IProducible, IStateful
{
     ...
     * @param {Types/_entity/Record} item Удаляемая запись
     ...
     * @return {Types/_entity:Record} Удаленная запись
     * @example
    ...
    static checkOptions<T>(options?: IOptions<T>): void {
        if (options) {
            if ('items' in options) {
                logger.stack(
    ...
    static es5Constructor<T>(options?: IOptions<T>): void {
        RecordSet.checkOptions(options);
