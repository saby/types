...
export interface ISession {
     ...
     *
     */
    removedProcessed?: Record<string, unknown>;
}

export interface IComparatorChanges {
    offset: number;
    endAt: number;
    oldItems: any[];
    newItems: any[];
 ...
 * @param collection Коллекция
 * @param startFromНачать с элемента номер
 * @param offsetСмещение элементов в after относительно before
 */
        ...
        switch (groupName) {
            ...
            case 'removed':
                ...
                do {
                    ...
                    if (afterIndex === -1) {
                        lookUp = false;
                    } else if (removedProcessed && removedProcessed[afterIndex]) {
                        // этот индекс мы уже находили, значит beforeItem - дубль, ищем дальше
                        skip = afterIndex + 1;
                    } else {
                        if (removedProcessed && !oldItems.length) {
                            // запомним найденный индекс
...
function applyGroupChanges(groupName: string, changes: any, session: ISession): void {
    ...
    switch (groupName) {
        case 'added':
            ...
            if (beforeContents !== null && afterContents) {
...
const enumerableComparator = {
    ...
    analizeSession(
        ...
        let changes: IComparatorChanges;
