        ...
        switch (groupName) {
            case 'added':
                ...
                if (beforeIndex === -1) {
                    ...
                    newItemsIndex =
                        newItems.length === 1 ? afterIndex : newItemsIndex;
            ...
            case 'replaced':
                ...
                if (
                    ...
                    oldItems.push(
                        (collection as any)
                            ._getItemsStrategy()
                            .convertToItem(beforeContents[index])
                    );
                    newItems.push(afterItem);
            ...
            case 'moved':
                ...
                if (beforeIndex !== afterIndex) {
                    if (
                        (oldItems.length &&
                            beforeIndex !== oldItemsIndex + oldItems.length) ||
                        (newItems.length &&
                            afterIndex !== newItemsIndex + newItems.length)
                    ) {
                        exit = true;
                    } else {
                        if (oldItems.length === 0) {
    ...
    switch (groupName) {
        ...
        case 'moved':
            const afterSpliceIndex =
                changes.oldItemsIndex + changes.oldItems.length >
                changes.newItemsIndex
                    ? changes.newItemsIndex
            ...
            if (
                changes.endAt !== -1 &&
                changes.oldItemsIndex < changes.newItemsIndex
            ) {
                changes.endAt -= changes.oldItems.length;
            }
            break;
