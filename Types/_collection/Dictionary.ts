    ...
    implements IEnumerable<T>, IEquatable
{
    ...
    getEnumerator(localize?: boolean): IEnumerator<T> {
        const dictionary =
            localize && this._$localeDictionary
                ? this._$localeDictionary
                : this._$dictionary;
        const enumerator =
    ...
    isEqual(to: any): boolean {
        ...
        do {
            ...
            if (
                enumerator.getCurrentIndex() !== toEnumerator.getCurrentIndex()
            ) {
                return false;
            }
        ...
        return (
            ((format as format.DictionaryField).getDictionary
                ...
                : (format as format.UniversalField).meta &&
                  (
                      (format as format.UniversalField)
                          .meta as format.IUniversalFieldDictionaryMeta
                  ).dictionary) || []
        );
    }
