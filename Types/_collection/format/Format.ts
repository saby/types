 ...
 * @class Types/collection:format.Format
 * @example
    ...
    implements IEquatable
{
    ...
    protected _checkItem(item: T): void {
        if (!item || !(item instanceof format.Field)) {
            throw new TypeError(
                'Item should be an instance of "Types/entity:format.Field"'
            );
        }
    }
    ...
    protected _checkName(item: any, at?: number): void {
        ...
        if (exists > -1 && exists !== at) {
            throw new ReferenceError(
                `${
                    this._moduleName
                }: field with name "${item.getName()}" already exists`
            );
        }
    }
     ...
     * </pre>
     * @name Types/collection:format.Format#each
     * @function
     * @public
     */
