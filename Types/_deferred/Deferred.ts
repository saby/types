...
const SKIP_CALLBACK_LOG =
    typeof Symbol === 'undefined' ? '$skipCallbackLog$' : Symbol('skipCallbackLog');
...
type TChainCallback<T, TResult = unknown> = [TCallback<T, TResult>, TErrBack<T, TResult>];
 ...
 */
function checkThreadMatch(reqUUID: string, logger: ILogger, creationStack?: string): void {
    if (!reqUUID) {
        return;
    }
    ...
    const err = new Error(
        `Deferred создан в рамках реквеста "${reqUUID}", но используется в "${currentReqUUID}".
        Стек создания Deferred-а: ${creationStack}`
    );
    logger.error(
        'Deferred',
        'Попытка взаимодействия с экземпляром Deferred, созданным в другом потоке.',
        err
    );
}
...
function isDeferredLikeValue<T>(instance: unknown): instance is Deferred<T> {
    return Boolean(
        instance && (instance as Deferred<T>).addCallback && (instance as Deferred<T>).addErrback
    );
}

function resultToFired(res: unknown): number {
    return isCancelValue(res) ? STATE.CANCELED : isErrorValue(res) ? STATE.FAILED : STATE.SUCCESS;
}
...
export default class Deferred<T> extends Promise<T> {
     ...
     * @param options.cancelCallback Каллбэк, вызывающийся при отмене Deferred.
     * @example
     * <pre>
     *    var dfr;
    ...
    constructor(options?: IOptions<T>) {
        ...
        this.initDeferred(options);
    }

    initDeferred(options?: IOptions<T>) {
        if (options) {
            if (options.cancelCallback) {
                this._cancelCallback = options.cancelCallback;
            }
            if (options.parentPromise) {
                this._parentPromise = options.parentPromise as IExtPromise<T>;
            }
        }

        this._reqUUID = getRequestUUID();

        if (this._reqUUID) {
            this._creationStack = new Error('Стек создания Deffered-а').stack;
        }

        this._chained = false;
        this._chain = [];
        this._fired = STATE.WAITING;
        this._paused = 0;
        this._results = [undefined, undefined];
        this._running = false;
        this._hasErrback = false;
        this._logger = options && options.logger;
        this._loggerAwait = (options && options.loggerAwait) || setTimeout;
    }
    ...
    static es5Constructor<T>(options?: IOptions<T>) {
        ...
        Deferred.prototype.initDeferred.call(this, options);
    }
     ...
     */
    cancel(): this {
        checkThreadMatch(this._reqUUID, this.logger, this._creationStack);
        ...
        if (this._fired === STATE.WAITING) {
            ...
            this._results[CHAIN_INDICES[this._fired]] = new DeferredCanceledError('Cancel');
    ...
    addBoth<TResult1 = T, TResult2 = never>(
        ...
        return this.addCallbacks(fn as TCallback<T, TResult1>, fn as TErrBack<T, TResult1>);
    }
    ...
    addCallbacks<TResult1 = T, TResult2 = never>(
        ...
        const waiting = fired === STATE.WAITING || this._running || this._paused > 0;
     ...
     * @param withChainПроверять, отработала ли цепочка обработчиков.
    ...
    isReady(withChain?: boolean): boolean {
        ...
        return this._fired !== STATE.WAITING && (withChain ? !this._paused : true);
    }
     ...
     */
        ...
        let callback: TCallback<T>;
        let errback: TErrBack<T>;

        // finally не поддерживается Node < 10
        const promise = new Promise<T>((resolve, reject) => {
            callback = resolve as TCallback<T>;
            errback = reject as TErrBack<T>;
        }).then(
            (res) => {
                onFinally?.();
                return res;
    ...
    then<TResult1 = T, TResult2 = never>(
        ...
        this.addCallbacks(
            ...
            Deferred.skipLogExecutionTime((res: TRes<T>) => {
                callback(res);
                return res;
            }),
            Deferred.skipLogExecutionTime((err: Error) => {
                errback(err);
                return err;
            })
        );
    ...
    protected _fire(): void {
        ...
        while (chain.length > 0 && this._paused === 0) {
            ...
            try {
                ...
                fired = STATE.FAILED;
                // @ts-ignore https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error/cause
                res = isErrorValue(err) ? err : new Error(err as string, { cause: { needShow: true } });
                this.logger.error(
                    'Deferred',
                    `Callback function throwing an error: ${(err as Error).message}`,
                    err as Error
                );
            } finally {
                this._running = false;
    ...
    protected _resback(res: TRes<T>): void {
    ...
    protected _check(res: TRes<T>, isError?: boolean): TRes<T> {
        let result: TRes<T> = res;
        if (this._fired !== STATE.WAITING) {
        ...
        if (isError) {
            if (!isErrorValue(result)) {
                // @ts-ignore https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error/cause
                result = new Error(String(result || ''), { cause: { needShow: true } });
            }
            ...
            if (!constants.isBrowserPlatform) {
                ...
                rejectionAwait(() => {
                    if (!this._hasErrback) {
                        ...
                        this.logger.error('Deferred', 'Unhandled error', result as Error);
                    }
                });
            }
        }
    ...
    static fail(result: Error | string): Deferred<Error> {
        // @ts-ignore https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error/cause
        const err = result instanceof Error ? result : new Error(result ? String(result) : '', { cause: { needShow: true } });
    ...
    static nearestOf<T>(steps: Deferred<T>[]): Deferred<INearestResult<T>> {
        ...
        steps.forEach((step: Deferred<T>, index) => {
            step.addBoth((stepResult: T) => {
                if (!result.isReady()) {
