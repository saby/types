 ...
 *     unregister('My/User');
 * </pre>
 * @see Types/_collection/RecordSet#getModel
 * @module Types/di
 * @public
 */

const SINGLETONE_MAP_INDEX = 2;
const map = new Map<string, [object | Function, IOptions, object?]>();
...
export function resolve<TResult, TOptions = IHashMap<any>>(
    ...
    switch (aliasType) {
        ...
        default:
            ...
            [Factory, config, singleInst] = map.get(alias as string);
    }
    ...
    if (config) {
        ...
        if (config.single === true) {
            if (singleInst === undefined) {
                singleInst = map.get(alias as string)[SINGLETONE_MAP_INDEX] = new Factory(options);
            }
            return singleInst;
        }
    }
