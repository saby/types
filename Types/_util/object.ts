...
import {
    getJsonReplacerWithStorage,
    getJsonReviverWithStorage,
} from '../formatter';
 ...
 */
export function setPropertyValue<T>(
    obj: unknown | IObject,
    property: string,
    value: T
): void {
    ...
    if (
        isImplementsIObject(obj) &&
        (obj as IObject).has(checkedProperty as string)
    ) {
    ...
    throw new ReferenceError(
        `Object doesn't have setter for property "${property}"`
    );
}
 ...
 * @param path Путь внутри объекта
 */
export function implantValue<T>(
    obj: unknown,
    path: string[],
    value: T
): boolean {
...
export function clone<T>(original: T | ICloneable): T {
    if (original instanceof Object) {
        if (original['[Types/_entity/ICloneable]']) {
            ...
            const reviver = getJsonReviverWithStorage(
                undefined,
                functionsStorage
            );
            return JSON.parse(JSON.stringify(original, replacer), reviver);
        }
    } else {
        return original;
    }
...
function clonePlainInner<T>(
    ...
    if (PLAIN_OBJECT_STRINGIFIER.call(original) === '[object Array]') {
        ...
        result = (original as unknown as T[]).map((item) => {
            return clonePlainInner<T>(item, options, inProgress);
        });
        ...
        if (Object.getPrototypeOf(original) === PLAIN_OBJECT_PROTOTYPE) {
            ...
            options.processCloneable &&
            original['[Types/_entity/ICloneable]']
        ) {
