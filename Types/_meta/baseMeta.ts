...
export class Meta<RuntimeInterface> {
    ...
    /**
     * Преобразует в json
     * TODO сделать отдельный сериализатор
     */
    toJSON(): unknown[] {
        const groupObj = this._info.group ? { group: [this._info.group.uid, this._info.group.name] } : {};
        // @ts-ignore
        const editor = this._propsEditor?.loader?._moduleName ? { editor: this._propsEditor.loader._moduleName } : {};
        const editorProps = this._propsEditor?.props ? { editorProps: JSON.stringify(this._propsEditor.props) } : {};
        // @ts-ignore
        const designtimeEditor = this._designtimeEditor?.loader?._moduleName ?
        // @ts-ignore
            { designtimeEditor: this._designtimeEditor.loader._moduleName } : {};
        // @ts-ignore
        const designtimeEditorProps = this._designtimeEditor?.props ?
            { designtimeEditorProps: JSON.stringify(this._designtimeEditor.props) } : {};
        return [{
            is: MetaClass.primitive,
            id: this._id || undefined,
     ...
     * @public
     */
    clone<NewRuntimeInterface = this>(update: Partial<IMeta<any>> = {}): NewRuntimeInterface {
        ...
        if (!this.fixedId && !update.id) {
            id = this._id + ('.' + Math.random()).substring(2, GENERATOR_ID_RANDOM_END);
     ...
     */
    designtimeEditorProps(
        props: Partial<Omit<IPropertyEditorProps<RuntimeInterface>, IPropertyEditorHiddenProps>> | undefined
    ): this {
    ...
    static registerChildMeta<T extends TMetaConstructor>(metaClass: T, condition: (item: unknown) => boolean): void {
