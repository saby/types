...
export class Meta<RuntimeInterface> {
     ...
     */
    toJSON(): unknown[] {
        const groupObj = this._info.group
            ? { group: [this._info.group.uid, this._info.group.name] }
            : {};
        // @ts-ignore
        const editor = this._propsEditor?.loader?._moduleName
            ? // @ts-ignore
              { editor: this._propsEditor.loader._moduleName }
            : {};
        const editorProps = this._propsEditor?.props
            ? { editorProps: JSON.stringify(this._propsEditor.props) }
            : {};
        // @ts-ignore
