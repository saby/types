...
const GENERATOR_ID_RANDOM_DELIMITER = '→';
...
export class Meta<RuntimeInterface> {
     ...
     */
    toJSON(): unknown[] {
        // TODO уже пора создавать сериализатор
        let sampleData: { i?: string, d?: RuntimeInterface };
        if (this._sampleData?.importPath) {
            sampleData = { i: this._sampleData.importPath };
        } else if (this._sampleData?.data) {
            // eslint-disable-next-line
            sampleData = { d: this._sampleData.data };
        }

        const group = this._info.group ? [this._info.group.uid, this._info.group.name] : undefined;
        // @ts-ignore
        const editor = this._propsEditor?.loader?._moduleName;
        const editorProps = this._propsEditor?.props ? JSON.stringify(this._propsEditor.props) : undefined;
        // @ts-ignore
        const designtimeEditor = this._designtimeEditor?.loader?._moduleName;
        const designtimeEditorProps = this._designtimeEditor?.props
            ? JSON.stringify(this._designtimeEditor.props)
            : undefined;
        ...
        Object.keys(selfJson).forEach((key) => {
            if (selfJson[key] !== undefined) {
                return;
            }
            delete selfJson[key];
        });
        selfJson.inherits = selfJson.inherits.filter((item) => {
