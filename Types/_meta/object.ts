 ...
 * @see IObjectMeta
 */
export class ObjectMeta<RuntimeInterface extends object> extends Meta<RuntimeInterface> {
    /**
     * Атрибуты объекта.
    ...
    toJSON(): any[] {
        ...
        metaResult[0].is = MetaClass.object;
        return ObjectMeta.toJSONAddAttributies(metaResult, this._attributes);
    }
    ...
    protected static toJSONAddAttributies(
        result: any[],
        attrObject: object): any[]
    {
        const basicType = result[result.length - 1];
        basicType.attributes = [];
        const attrResults = [];
        Object.entries(attrObject).forEach(([key, value]: [string, Meta<any>]) => {
            basicType.attributes.push([value.getId(), key]);
            attrResults.push(value.toJSON());
        });
        return Array.prototype.concat.apply([], attrResults.concat(result));
    }
