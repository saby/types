...
export class ObjectMeta<RuntimeInterface extends object> extends Meta<RuntimeInterface> {
    ...
    toJSON(): unknown[] {
        ...
        const basicType = metaResult[0];

        basicType.attributes = [];
        const attrResults = [];
        Object.entries(this._attributes).forEach(([key, value]: [string, Meta<any>]) => {
            basicType.attributes.push([value.getId(), key]);
            attrResults.push(value.toJSON());
        });

        return Array.prototype.concat.apply([basicType], attrResults);
    }
