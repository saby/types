    ...
    async load(value?: unknown): Promise<IComponent<ComponentPropsInterface>> {
        if (this.loader && this.component === undefined) {
            ...
            try {
                ...
                logger.error(
                    'Types/meta: Ошибка загрузчика компонента редактора: ',
...
> extends ComponentLoaderWithProps<ComponentPropsInterface, PropsInterface> {
    ...
    async load(value?: unknown): Promise<IComponent<ComponentPropsInterface>> {
        if (!this.ready) {
            ...
            await Promise.all(
                Object.values(this._attributes || {}).map(async (meta) => {
                    if (!meta) {
                        return;
                    }
                    const originEditor = this.editorGetter(meta.getOrigin()?.meta);
                    ...
                    try {
                        ...
                        logger.error('Types/meta: Редактор был пропущен из-за ошибки загрузки');
...
export class TmpMetaEditor implements IMetaEditor {
    ...
    constructor(loader: string | Function, public props?: object, public component?: Component) {
        if (typeof loader === 'string') {
