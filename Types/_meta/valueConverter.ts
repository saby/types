    ...
    async load(value?: unknown): Promise<FS> {
        if (this.loader && this.func === undefined) {
            ...
            try {
                ...
                logger.error(
                    'Types/meta: Ошибка загрузчика функции конвертера: ',
            ...
            if (typeof result === 'function') {
                ...
                throw new Error(
                    'Неизвестный формат функции конвертера: ' + result
                );
            }
        }
        return this.func;
...
export const BROKEN_FUNC: any = () => {
    logger.error(
        'Из-за ошибки загрузки функции конвертера использована функция-заглушка'
    );
};

export type ConversionContent = (
    | IEditingElementFacade
    | IEditingElementFacade[]
)[];
 ...
 * @public
 */
export type ValueConverterLoader<
    FS extends ValueConverterOutputType | ValueConverterInputType
> = FuncLoader<FS>;
...
export interface IConvertableChecker
    extends ILoadableConverterFunc<ConvertableCheckerSignature> {}
