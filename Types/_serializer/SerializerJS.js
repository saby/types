...
define('Types/_serializer/SerializerJS', [
   ...
   var NEW_TYPE_META = SerializerTypes.NEW_TYPE.META;
   ...
   Serializer.serializeWith = function (self, throwable) {
      return function (name, value) {
         ...
         if (isPosiblyMetaType(value, Serializer.metaClass)) {
            var res = { state: JSON.stringify(value) };
            res[SERIALIZED_KEY] = NEW_TYPE_META;
            return res;
         }
   ...
   /*
    * Функция десериализации мета типов.
    * Должен задаваться из вне, т.к. Types зависит от Meta.
    */
   Serializer.deserializeMetaType = function (value) {
      return value;
   };
   ...
   Serializer.setMetaHandler = function (deserialize, metaCLass) {
      Serializer.deserializeMetaType = deserialize;
      Serializer.metaClass = metaCLass;
   };
   ...
   Serializer.deserializeWith = function (self) {
      return function (name, value) {
         ...
         if (isObject && value.hasOwnProperty(SERIALIZED_KEY)) {
            switch (value[SERIALIZED_KEY]) {
               case TYPE_FUNC:
                  ...
                  break;
               case NEW_TYPE_META:
                  result = Serializer.deserializeMetaType(JSON.parse(value.state));
                  break;
               case TYPE_INST:
         ...
         if (name === 'PrefetchLoader' && isObject) {            
            Serializer.findMeta(value);
         }
   ...
   /**
    * Рекурсивная десериализация метатипов
    * @param {unknown} currentElement 
    * @returns 
    */
   Serializer.findMeta = function (currentElement) {
      if (!(currentElement instanceof Object) || isOriginMeta(currentElement) || currentElement === null) {
         return;
      }
      for (var key in currentElement) {
         if (!currentElement[key] || (currentElement[key] && currentElement[key].hasOwnProperty(SERIALIZED_KEY))) {
               continue;
         }                   
         if (currentElement[key][0] instanceof Object && Serializer.metaClass.hasOwnProperty(currentElement[key][0].is)) {
            currentElement[key] = Serializer.deserializeMetaType(currentElement[key]);
            continue;
         }
         Serializer.findMeta(currentElement[key]);
      }    
   }
   ...
   /**
    * Проверка, что значение возможно какой-то мета тип
    */
   function isPosiblyMetaType(value, metaClass) {
      if (!value || !Array.isArray(value) || value.length === 0) {
         return false;
      }

      var last = value[value.length - 1];
      return last && last.is && Object.values(metaClass).includes(last.is);
   }
   
   /**
    * Проверка, что значение meta.origin
    * такие значения не десериализуем, т.к. они десериализуются внутри Meta
    */
   function isOriginMeta(property) {
      return !!(property.meta && property.key);
   }
