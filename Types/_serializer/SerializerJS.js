...
define('Types/_serializer/SerializerJS', [
   ...
   var NEW_TYPE_DATE = SerializerTypes.NEW_TYPE.DATE;
      ...
      return function (name, value) {
         ...
         if (typeofValue === 'function') {
            ...
            if (!value._moduleName && !!throwable) {
               throw new Error(
                  'Невозможно сериализовать функцию (с prototype.moduleName = ' +
                     (value.prototype && value.prototype._moduleName) +
   ...
   Serializer._patterns = [
      {
         // name - имя шаблона десериализации. необходимо для того, чтобы можно было отключать указанные шаблоны
         // например в StateReceiver
         name: 'DATE',
   ...
   /**
    * Список имен шаблонов из Serializer._pattern, которые будут исключены из проверки при десериализации
    */
   Serializer._ignorePatternNames = new Set();

   Serializer.setIgnorePatternName = function setIgnorePatternName(ignoreName) {
      Serializer._ignorePatternNames.add(ignoreName);
   };

   Serializer.deleteIgnorePatternName = function deleteIgnorePatternName(ignoreName) {
      Serializer._ignorePatternNames.delete(ignoreName);
   };
         ...
         if (isObject && value.hasOwnProperty(SERIALIZED_KEY)) {
            switch (value[SERIALIZED_KEY]) {
               case TYPE_FUNC:
                  ...
                  break;
               case NEW_TYPE_DATE:
                  result = deserializeDate(value.state);
                  break;
               case TYPE_INST:
            ...
            switch (result) {
               ...
               default:
                  for (
                     ...
                     if (pattern.name && Serializer._ignorePatternNames.has(pattern.name)) {
                        continue;
                     }
   ...
   function deserializeDate(result) {
