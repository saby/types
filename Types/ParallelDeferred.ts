...
import { Deferred, TErrBack } from 'Types/deferred';
import { IParallelDeferredOptions, deferredFunction } from './IParallelDeferred'
...
class ParallelDeferred {
    protected _successResult: unknown;
    ...
    protected _results: Record<string, unknown> = {};
    protected _lazyQueue: [Deferred<unknown> | deferredFunction, string | number][] = [];
     ...
     * Конструктор класса
     */
    constructor(cfg?: IParallelDeferredOptions) {
        this._req =
            //@ts-ignore
            globalThis.process && globalThis.process.domain && globalThis.process.domain.req;
    ...
    private _runDfr(
        lazyDfr: deferredFunction | Deferred<unknown> | undefined,
        isLazy: boolean,
        ...
        (dfr as Deferred<unknown>)?.addCallbacks(
            this._successHandler.bind(this, stepId),
            this._errorHandler.bind(this, stepId) as TErrBack<unknown>
        );
    }

    /**
     * Добавляет Deferred в набор.
     * @param dOperation Асинхронная операция для добавления в набор.
     ...
     * @param stepId  Идентификатор шага (операции). Результат шага (операции) с заданным идентификатором будет
    ...
    push(
        ...
        const self = this;
        const isEager = dOperation instanceof Deferred || dOperation instanceof Promise;
     ...
     * @param successResult Результат, который будет возвращен в случае успеха в общий callback.
     * @returns
     * @see {@link getStepsDone}
     */
    ...
    _check(): void {
        const checkQueue = () => {
            ...
            const needUnqueue = queueLn > 0 && runningCnt < this._options.maxRunningCount;
            ...
            if (this._ready) {
                ...
                if (this._isFinishedByError()) {
                    ...
                    this._dResult.callback(
                        this._successResult !== undefined ? this._successResult : this._results
                    );
                }
            } else {
     ...
     * @returns Число операций в наборе.
     * @see {@link push}
     * @see {@link getStepsDone}
     * @see {@link getStepsSuccess}
     */
     ...
     * @returns Число операций, оконченных успешно и безуспешно.
     * @see {@link push}
     * @see {@link getStepsSuccess}
     * @see {@link getStepsCount}
     */
    getStepsDone(): number {
     ...
     * @returns Число успешно оконченных операций.
     * @see {@link push}
     * @see {@link getStepsCount}
     * @see {@link getStepsDone}
     */
