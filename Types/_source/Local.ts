...
export type MemoryFilterFunction = (
    ...
    implements ICrud, ICrudPlus
{
    ...
    move(
        ...
        items
            .sort((a, b) => {
                ...
                return meta.position === MOVE_POSITION.After
                    ? indexB - indexA
                    : indexA - indexB;
            })
            .forEach((id) => {
    ...
    getAdapter(): adapter.IAdapter {
        ...
        if (
            ...
            (
                this._$adapter as unknown as adapter.IDataHolder<TData>
            ).dataReference = this.data;
    ...
    protected _applyWhere(
        ...
        this._each(data, (item, index) => {
            ...
            if (this._$filter) {
                ...
                isMatch = isPredicate
                    ? (where as Function)(item, index)
                    : checkFields(where, item);
            }
    ...
    protected _applyOrderBy(data: any, order: Order[]): any {
        ...
        dataMap.sort((a, b) => {
            ...
            for (let index = 0; index < orderMap.length; index++) {
                result =
                    (orderMap[index].order ? -1 : 1) *
                    compare(a.values[index], b.values[index]);
                if (result !== 0) {
                    break;
                }
            }
    ...
    protected _reorderMove(
        ...
        items.forEach((item) => {
            ...
            if (meta.position === MOVE_POSITION.Before && targetIndex > index) {
                ...
                meta.position === MOVE_POSITION.After &&
                targetIndex < index
            ) {
                targetIndex++;
            }
