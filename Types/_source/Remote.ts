...
import OptionsMixin, {
    IOptionsOption as IOptionsMixinOption,
} from './OptionsMixin';
    ...
    if (this._$options.updateOnlyChanged) {
        ...
        if (!isEmpty(keyProperty)) {
            if (
                ...
                !isNull((data as Record).get(keyProperty))
            ) {
    ...
    implements ICrud, ICrudPlus, IProvider
{
     ...
     * <pre>
     *     import {RPCRequestParam} from 'Types/source';
     ...
     *     });
     * </pre>
     * Пример обработчика сообщений, выставляющего таймаут запроса в 2 мин на уровне транспорта:
     * <pre>
     *     import {RPCRequestParam} from 'Types/source';
     *     // объявляем обработчик сообщений
    ...
    protected initRemote(options?: IOptions) {
        ...
        if (!!options?.callbacks?.beforeProviderCall) {
            this._beforeProviderCallCallback =
                options.callbacks.beforeProviderCall;
        }
    ...
    protected _notifyBeforeProviderCall(name: string, args: object): object {
        ...
        if (!!this._beforeProviderCallCallback) {
            callbackFunction = this._resolveCallback(
                this._beforeProviderCallCallback
            );
        }
    ...
    protected _resolveCallback(moduleName: string): Function {
    ...
    static fromJSON<T = Remote, K = IOptions>(
    ...
    _getSerializableState(
        ...
        resultState._beforeProviderCallCallback =
            this._beforeProviderCallCallback;
    ...
    _setSerializableState(state: ISerializableState): Function {
        ...
        return function (): void {
            ...
            this._beforeProviderCallCallback =
                state._beforeProviderCallCallback;
        };
    }
    // endregion
