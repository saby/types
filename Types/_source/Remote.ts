...
/**
 * Методы подготовки аргументов при вызове удаленного источника.
 * @public
 */
...
export interface IOptionsOption extends IOptionsMixinOption {
     ...
     * @remark
     * Задавать опцию имеет смысл только если указано значение опции {@link Types/source:Remote#getKeyProperty}, позволяющая отличить новые записи от уже существующих.
     */
    updateOnlyChanged?: boolean;
    /**
     * Тип навигации, используемой в методе {@link query}.
     * @deprecated Вместо опции на источнике задайте мета-данные на методе {@link Types/source:Query#meta query}
     */
...
export interface IOptions
    extends IBaseOptions,
        ...
        IEndpointOptions {
    /**
     * Дополнительные настройки удаленного источника данных
     */
    ...
    /**
     * Обработчики аргументов вызова удаленного источника.
     */
    callHandlers?: List<ICallHandler>;
    /**
     * Функции обратного вызова для источника.
     */
    ...
    implements ICrud, ICrudPlus, IProvider
{
     ...
     * @remark Обработчики сообщений позволяют модифицировать параметры запроса на уровне транспорта.
     * @example
     ...
     * @param meta
     */
    copy(key: EntityKey, meta?: IHashMap<unknown>): Promise<Model> {
    ...
    protected _callProvider<TResult>(
        ...
        result.catch((error) => {
            if (methodNotExistsError(error)) {
                const message = `Method ${name} was not found on ${JSON.stringify(
                    this.getEndpoint()
                )}`;
                logger.stackToCloud(message, 2, 'error');
            }
...
function methodNotExistsError(error: any): boolean {
    const HTTP_STATUS_NOT_FOUND = 404;

    return (
        error.httpError === HTTP_STATUS_NOT_FOUND &&
        error?.details?.startsWith('The method ') &&
        error?.details?.endsWith(' is not found or accessible.')
    );
}
