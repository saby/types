...
interface IQueryEntriesOptions {
    marked: string[];
    excluded: string[];
    type: 'all' | 'leaf' | 'node';
    recursive: boolean;
    ...
    implements IDecorator, ICrud, ICrudPlus
{
                        ...
                        .then((response) => {
                            if (this._$parentProperty) {
                                ...
                                const whereConditions =
                                    preparedQuery.getWhere();
                                if (this._$parentProperty in whereConditions) {
                                    const startFromId =
                                        whereConditions[this._$parentProperty];
                                    let startFromNode =
                                        sourceRecords.getRecordById(
                                            startFromId
                                        );
                                    if (startFromNode) {
                                ...
                                const data =
                                    response.getRawData() as IQueryRawData;
                                ...
                                if (entries) {
                                    entryPath = this._calcEntryPath(
                                        sourceRecords,
                                        entries as Record<IQueryEntriesOptions>
                                    );
                                }
    ...
    _calcEntryPath(
        ...
        filter: Record<IQueryEntriesOptions>
    ): IEntryPath[] {
        ...
        if (filter.has(MARKED_FIELD)) {
            filter.get(MARKED_FIELD).forEach((entryId) => {
                let node = sourceRecords.getRecordById(entryId);
                let parentNode;
                while (
                    node &&
                    (parentNode = hierarchy.getParent(node, sourceRecords))
                ) {
                    entryPath.push({
    ...
    _prepareQuery(query?: Query): [Query, object] {
        if (!query) {
            return [query, null];
        }
        ...
        if (whereConditions && typeof whereConditions === 'object') {
            if ('entries' in whereConditions) {
                preparedEntries = whereConditions.entries;
            }
