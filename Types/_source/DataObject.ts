import { default as SbisService, IQueryResult, passQuery } from './SbisService';
...
interface IDataObjectQueryResult extends IQueryResult {
    /**
     * Название датасета
     */
    Название?: string;
...
export default class DataObject
    extends mixin<OptionsToPropertyMixin, SerializableMixin, OptionsMixin, DataMixin>(
        OptionsToPropertyMixin,
        SerializableMixin,
        OptionsMixin,
        DataMixin
    ...
    implements ICrud
{
    ...
    read<TData = DataSet<Model>>(key: CrudEntityKey | null, meta: IReadMeta = {}): Promise<TData> {
        ...
        const objectKey = key !== null ? String(key) : key;

        return this._callProvider<Promise<TData>>(
            this._$binding.read,
            passRead.call(this, objectKey, meta)
        );
    }
    ...
    query<TData = DataSet>(query?: Query): Promise<TData> {
        ...
        return this._callProvider<Promise<TData>>(
            ...
            this.passQuery.call(this, query as Query)
        );
    }

    passQuery(query: Query): IDataObjectQueryResult {
        const originalPassResult: IDataObjectQueryResult = passQuery.call(this.getSource(), query);
        if (this._$objectName) {
            originalPassResult['Название'] = this._$objectName;
        }

        // TODO: убрать проверку после решения задачи
        // https://online.sbis.ru/opendoc.html?guid=79370785-53ef-4be3-91e3-47183717c1f2&client=3
        if (!Object.keys(query.getWhere()).length) {
            // @ts-ignore
            originalPassResult['Фильтр'] = null;
        }

        return originalPassResult;
    }
