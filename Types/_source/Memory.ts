 ...
 */
const $cachedAdapter = Symbol('cachedAdapter');
...
/**
 * Интерфейс опций источника данных {@link Types/source:Memory}}
 * @public
 */
export interface IOptions extends ILocalOptions {
    ...
    data?: any;
    /**
     *  Описание конечной точки, обеспечивающей доступ клиента к функциональным возможностям провайдера удаленного доступа
     */
    endpoint?: IEndpoint;
}
...
export default class Memory extends Local {
    ...
    protected _prepareQueryResult<TData = DataSet>(data: any, query: Query): TData {
        ...
        return super._prepareQueryResult<TData>(data);
    }
    ...
    protected _getTableAdapter(): adapter.ITable {
        if (!this[$cachedAdapter]) {
            this[$cachedAdapter] = this.getAdapter().forTable(this._$data);
        }
    ...
    protected _getEmptyData(query?: Query): any {
        ...
        if (!this._emptyData.has(table as string)) {
            const items = object.clonePlain(this._applyFrom(table));
            const adapter = this.getAdapter().forTable(items);

            adapter.clear();
            this._emptyData.set(table as string, adapter.getData());
        }

        return this._emptyData.get(table as string);
    }
