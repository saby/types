 ...
 * @property {Boolean} [updateOnlyChanged=false] При сохранении отправлять только измененные записи (если обновляется набор записей) или только измененные поля записи (если обновляется одна запись). Задавать опцию имеет смысл только если указано значение опции {@link Types/_source/Remote#keyProperty}, позволяющая отличить новые записи от уже существующих.
 * @property {Types/source:SbisService.NavigationTypes.typedef} [navigationType] Тип навигации, используемой в методе {@link query}.
 */
...
export interface IOptionsOption extends IRemoteOptionsOption {
    /**
     * Название свойства мета-данных {@link Types/_source/Query#meta запроса}, в котором хранится значение поля HasMore аргумента Навигация, передаваемое в вызов {@link Types/_source/SbisService#query}.
     */
    hasMoreProperty?: string;
    /**
     * Передавать аргумент "ДопПоля" при вызове методов {@link Types/_source/SbisService#read} и {@link Types/_source/SbisService#update}, значение которых получено из метаданных {@link Types/_source/Query#meta запроса}.
     */
...
function buildRecord(data: unknown, adapter: AdapterDescriptor): Record | null {
    ...
    return RecordType.fromObject(data as object, adapter);
}
...
function buildRecordSet<T = unknown>(
    ...
    const RecordSetType = resolve<typeof RecordSet>(
...
function passUpdate(
    ...
    const superArgs = (Rpc.prototype as any)._$passing.update.call(
        this,
        data,
        meta
    );
    const args: IUpdateResult = {};
    const recordArg = DataMixin.isRecordSetInstance(superArgs.data)
        ? 'Записи'
        : 'Запись';
    const passAddFieldsFromMeta = this._$options.passAddFieldsFromMeta;
 ...
 */
export function getQueryArguments(
    query?: Query,
    options?: IOptions
): IQueryResult {
...
function oldMove(
    ...
    const moveMethod = meta.before
        ? this._$binding.moveBefore
        : this._$binding.moveAfter;
    const params = {
    ...
    params[meta.before ? 'ИдОДо' : 'ИдОПосле'] = createComplexId(
        to,
        this._$endpoint.contract
    );
...
export default class SbisService extends Rpc {
    ...
    /**
     * @cfg {Object.<Function>} Обработчики событий.
     * @name Types/_source/SbisService#handlers
     */
    ...
    /**
     * @cfg {Types/source:IBinding} Соответствие методов CRUD методам БЛ. Определяет, какой метод объекта БЛ соответствует каждому методу CRUD.
     ...
     * @return {Types/deferred:Deferred} Асинхронный результат выполнения: в случае успеха вернет {@link Types/_entity/Model}, в случае ошибки - Error.
     ...
     * @param {Object|Types/entity:Record} [meta] Дополнительные мета данные, которые могут понадобиться для удаления записи
    ...
    destroy(
        ...
        return Promise.all(
            Object.keys(groups).map((name) => {
                return callDestroyWithComplexId(groups[name], name, meta);
            })
        ...
        return Promise.all(
            Object.keys(groups).map((name) => {
                ...
                const methodName = buildBlMethodName(
                    this._$endpoint.moveContract,
                    this._$binding.move
                );
                const params = this._$passing.move.call(
                    this,
                    groups[name],
                    target,
                    meta
                ...
                return this._callProvider(methodName, params, handlers);
            })
        ) as unknown as Promise<void>;
    }
    ...
    static es5Constructor() {
        Rpc.es5Constructor.apply(this, arguments);

        SbisService.prototype.initSbisService.apply(this, arguments);
    }
}
