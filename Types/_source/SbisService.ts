...
/**
 * Опции источника данных {@link Types/source:SbisService}
 * @public
 */
 ...
 */
export function passQuery(this: SbisService, query?: Query): IQueryResult {
 ...
 *     query.limit(100);
 *     query.meta({
 *         avigationType: SbisService.NAVIGATION_TYPE.POSITION
 *     });
 *
 *     dataSource.query(query).then((response) => {
 *         const articles = response.getAll();
...
export default class SbisService extends Rpc {
     ...
     * @remark
     * Если задана функция обратного вызова beforeProviderCall, событие отключается.
     * @example
...
Object.assign(SbisService.prototype, {
