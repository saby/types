...
function getFilterParams(query: Query): object[] {
    ...
    eachQuery(query, (subQuery) => {
        ...
        const whereObject: IHierarchyOptions =
            whereExpr instanceof Record ? whereExpr : expressionToObject(whereExpr);
...
export default class SbisService extends Rpc {
     ...
     * @param {Object} [meta] Дополнительные мета данные, которые могут понадобиться для создания модели.
    ...
    update<TData = unknown>(data: Record | RecordSet, meta?: IHashMap<unknown>): Promise<TData> {
