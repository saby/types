...
export function andExpression<T>(
    ...conditions: (T | PartialExpression<T>)[]
): AndExpression<T> {
    return new AndExpression(conditions);
}

export function orExpression<T>(
    ...conditions: (T | PartialExpression<T>)[]
): OrExpression<T> {
    return new OrExpression(conditions);
}

type AtomAppearCallback<T = unknown> = (
    key: string,
    value: T,
    type: string
) => void;
type GroupBeginCallback<T> = (
    type: string,
    conditions: (T | PartialExpression<T>)[]
) => void;
...
function playExpressionInner<T>(
    ...
    if (expression instanceof AtomExpression) {
        ...
        if (onGroupEnds) {
            onGroupEnds(
                expression.type,
                stack.length && stack[stack.length - 1].type
            );
        }
    }
}
...
export class Order extends OptionsToPropertyMixin {
    ...
    getNullPolicy(): boolean {
        return this._$nullPolicy === undefined
            ? !this.getOrder()
            : this._$nullPolicy;
    }
    ...
    union(...queries: Query<T>[]): this {
        queries.forEach((q, i) => {
            if (!(q instanceof Query)) {
                throw new TypeError(
                    `Argument "queries[${i}]" should be an instance of Query.`
                );
            }
        });
                ...
                for (let i = 0; i < selector.length; i++) {
                    ...
                    if (selectorItem instanceof Array) {
                        const [
                            ...
                            selectorOrder,
                            selectorNullPolicy,
                        ]: [string, boolean, boolean] = selectorItem;
