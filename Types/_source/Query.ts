...
export interface IMeta extends Record<string, unknown> {
     ...
     * @remark
     * По умолчанию используется поле с именем hasMore. Однако, имя поля может быть изменено при настройке {@link Types/source:ISbisServiceOptions опции} hasMoreProperty в источнике данных.
     */
    hasMore?: boolean;
}
 ...
 *
 */
export type FilterExpression = Record<string, unknown> | FilterFunction<unknown>;
 ...
 *
 */
export type SelectExpression = Record<string, string> | string[] | string;
 ...
 *
 */
export type AtomAppearCallback<T = unknown> = (key: string, value: T, type: string) => void;
/**
 *
 */
export type GroupBeginCallback<T> = (
    type: string,
    conditions: (T | PartialExpression<T>)[]
) => void;
/**
 *
 */
export type GroupEndCallback = (type: string, restoreType: string) => void;

function playExpressionInner<T>(
    expression: PartialExpression<T>,
    onAtomAppears: AtomAppearCallback,
    onGroupBegins: GroupBeginCallback<T> | undefined,
    onGroupEnds: GroupEndCallback | undefined,
    ...
    if (expression instanceof AtomExpression) {
        ...
        // Play each condition
        expression.conditions.forEach((condition: T | PartialExpression<T>) => {
        ...
        if (onGroupEnds) {
            const restoreType = (stack.length && stack[stack.length - 1].type) || '';
            onGroupEnds(expression.type, restoreType);
        }
    }
}
...
export class Join extends OptionsToPropertyMixin {
    /**
     * Правильно заданное имя.
     */
    protected _$resource: string = '';

    /**
     * Псевдоним правильно установленного имени.
     */
    protected _$as: string = '';

    /**
     * Присоединить правило.
     */
...
export class Order extends OptionsToPropertyMixin {
    /**
     * Имя поля для применения сортировки.
     */
    ...
    /**
     * Политика позиционирования значений, подобных NULL (не определено - в зависимости от опции 'order', false - в начале, true - в конце).
     */
    protected _$nullPolicy: boolean | undefined = undefined;
...
export default class Query<T = unknown> implements ICloneable {
     ...
     * @param innerВнутреннее или внешнее соединение.
     * @example
     * <pre>
     *     import {Query} from 'Types/source';
     *
    ...
    join(
        ...
        if (!(resultName instanceof Array)) {
            throw new Error('Invalid argument "name"');
        }

        this._join.push(
            new Join({
                resource: resultName.shift() || '',
                as: resultName.shift() || '',
                on,
     ...
     * @return Определение способа сортировки
     * @example
     * Подробнее о выставлении правил сортировки читайте в описания метода {@link Types/source:Query#orderBy orderBy}
     * Предположим, что у нас есть правило сортировки, заданное следующим образом:
     * <pre>
     *     import { Query } from 'Types/source';
     *
     *     // заказы, отсортированные по возрастанию значения поля id
     *     // сортировка по возрастанию - поведение по-умолчанию
     *     // сортировку по убыванию рассмотрим в примере ниже
     *     const ordersByAsc = new Query()
     *         .from('Orders')
     *         .orderBy('id');
     * </pre>
     * <pre>
     *     import { Query } from 'Types/source';
     *
     *     // заказы, отсортированные по убыванию значения поля id
     *     // Чтобы отсортировать по убыванию, вторым аргументом в метод orderBy передадим true
     *     const ordersByDesc = new Query()
     *         .from('Orders')
     *         .orderBy('id', true);
     * </pre>
     * Получим правила сортировки для первого запроса:
     * <pre>
     *     const order = ordersByAsc.getOrderBy()[0];
     *     console.log(order.getSelector()); // 'id'
     *     console.log(order.getOrder()); // false - поле сортируется по возрастанию
     * </pre>
     *
     * Получим правила сортировки для второго запроса:
     * <pre>
     *     const order = ordersByAsc.getOrderBy()[0];
     *     console.log(order.getSelector()); // 'id'
     *     console.log(order.getOrder()); // true - по убыванию
     * </pre>
     */
    getOrderBy(): Order[] {
     ...
     * @param selector Имя поля для сортировки.
     * @param desc Направление сортировки (false - по возрастанию (по-умолчанию), true - по убыванию).
     * @param nullPolicy Политика позиционирования значений, подобных NULL (false - в начале, true - в конце).
     * @example
    ...
    orderBy(selector: OrderSelector, desc?: boolean, nullPolicy?: boolean): this {
        ...
        if (typeof selector === 'object') {
            const processObject = (obj: IHashMap<boolean>) => {
                if (!obj) {
                    return;
                }
     ...
     * </pre>
     * @returns Дополнительные метаданные
     */
    getMeta<U = IMeta>(): U {
