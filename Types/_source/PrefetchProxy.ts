...
export interface IDone {
    /**
     * Предварительно выбранные данные для метода чтения записи.
     */
    read?: boolean;
    /**
     * Предварительно выборанные данные для метода чтения списка записей.
     */
    query?: boolean;
    /**
     * Предварительно выбранные данные для метода копирования записи.
     */
    copy?: boolean;
}
...
export interface IValidators {
    /**
     * Валидатор предварительно выбранных данных для метода чтения записи.
     */
    read?: (data: Record, done?: IDone, key?: EntityKey, meta?: object) => boolean;
    /**
     * Валидатор предварительно выбранных данных для метода чтения списка записей.
     */
    query?: (data: DataSet | Error, done?: IDone, query?: Query) => boolean;
    /**
     * Валидатор предварительно выбранных данных для метода копирования записи.
     */
    copy?: (data: Record, done?: IDone, key?: EntityKey, meta?: object) => boolean;
}

/**
 * Интерфейс опций класса PrefetchProxy
 * @public
 */
export interface IOptions {
    /**
     * Целевой источник данных. Запрос отправляется к источнику, если предварительно выбранные данные больше недействительны.
     */
    target: ITarget;
    /**
     * Предварительно выбранные данные для методов, которые обеспечивают операции чтения.
