...
import { ExtendDate, IExtendDateConstructor } from 'Types/declarations';

function jsonizePlainObject(obj: Record<string, unknown>): Record<string, unknown> {
    ...
    if (proto !== null && proto !== Object.prototype) {
        const info = {
            ...
            moduleName: obj?._moduleName,
        };
        throw new TypeError(
            `Unsupported object type ${JSON.stringify(
                info
            )}. Cannot serialize object for network request. Only plain objects, Records and RecordSets can be serialized`
        );
    }
    ...
    if (obj.hasOwnProperty('toJSON') && obj.toJSON !== result.toJSON) {
        ...
        if (obj instanceof TheDate) {
            ...
            switch ((obj as ExtendDate).getSQLSerializationMode()) {
                ...
                case (Date as IExtendDateConstructor).SQL_SERIALIZE_MODE_TIME:
                    mode = TO_SQL_MODE.TIME;
                    break;
                case (Date as IExtendDateConstructor).SQL_SERIALIZE_MODE_DATETIME:
                    mode = TO_SQL_MODE.DATETIME;
                    break;
            }
        }
