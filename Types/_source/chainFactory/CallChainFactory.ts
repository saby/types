 ...
 * @param {Types/collection:List<Types/source:ICallHandler>} handler пользовательский обработчик сообщений
 */
...
export class ChainFactory extends mixin<DestroyableMixin>(DestroyableMixin) {
     ...
     * @param {Types/collection:List<Types/source:ICallHandler>} handlers список пользовательских обработчиков сообщений
     */
     ...
     * @param {Types/collection:List<Types/source:ICallHandler>} handlers список пользовательских обработчиков сообщений.
     * @returns {Boolean} Признак обновления цепочки обработчиков сообщений
     * @public
     */
     ...
     * @param {Types/collection:List<Types/source:ICallHandler>} handler пользовательский обработчик сообщений
     */
    private _generateChainItem(
        handler?: ICallHandler,
        nextItem?: ICallChainItem
    ): ICallChainItem {
        if (!handler && !nextItem) {
            return new RootCallChainItem();
        }

        return new CallChainItem(handler, nextItem);
    }
    ...
    isChanged(handlers: List<ICallHandler>): boolean {
        if (handlers.getVersion() !== this.getChainVersion()) {
            return true;
        }
     ...
     * @returns {number} Версия списка обработчиков
     */
