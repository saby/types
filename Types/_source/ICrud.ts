...
export default interface ICrud {
     ...
     * @return Асинхронный результат выполнения: в случае успеха вернет {@link Types/entity:Record} - созданную запись, в случае ошибки - Error.
     ...
     * @return Асинхронный результат выполнения: в случае успеха вернет {@link Types/entity:Record} - прочитанную запись, в случае ошибки - Error.
     * @example
     ...
     * Если у SbisService выставлена опция {@link Types/source:ISbisServiceOptionsOption passAddFieldsFromMeta}, то любые мета-данные будут передаваться в виде аргумента "ДопПоля" на метод БЛ.</br>
     * Существуют следующие ограничения:
     * <ul>
     *     <li>Аргумент "ДопПоля" не используется в автогенериуемых методах;</li>
     *     <li>Аргумент "ДопПоля" передается только при вызове методов {@link Types/source:SbisService#read read} {@link Types/source:SbisService#update update};</li>
     *     <li>Если не передан параметр meta, то  используются значение из мета-данных {@link Types/source:Query#meta запроса}.</li>
     * </ul>
     */
