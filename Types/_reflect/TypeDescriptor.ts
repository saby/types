    ...
    for (const sourceName in parentSource) {
        ...
        if (!source.hasOwnProperty(sourceName)) {
            ...
            source[sourceName].reference ===
                parentSource[sourceName].reference ||
            !source[sourceName].reference
        ) {
            source[sourceName] = merge(
                source[sourceName],
...
export class TypeDescriptor {
    ...
    addType(metadata: IType): void {
        if (this.hasType(metadata.typeId)) {
            throw new Error(
                `Тип с идентификатором ${metadata.typeId} уже существует`
            );
        }
    ...
    implements ISerializableState
{
    ...
    private _initStateReceiver(): void {
        const store: IStore<Record<string, boolean>> =
            getStore(STORE_REGISTERED_KEY);
