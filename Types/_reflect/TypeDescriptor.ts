...
function merge(
    ...
    parentProp: object = {},
    specificProps?: {
        // Здесь осознанно стоит any, потому что правильно здесь не типизировать
        // Типизация должна быть на уровне самих коллбеков
        [name: string]: (prop: any, parentProp: any, key?: string) => unknown;
    }
): unknown {
    for (const key in parentProp) {
        if (specificProps && specificProps[key]) {
            ...
            // @ts-ignore
            if (Array.isArray(parentProp[key])) {
                // @ts-ignore
                prop[key] = mergeArrays(prop[key], parentProp[key]);
            } else {
                // @ts-ignore
                if (typeof parentProp[key] === 'object') {
                    // @ts-ignore
...
function mergeSources(
    ...
    for (const sourceName in parentSource) {
        ...
        if (!source.hasOwnProperty(sourceName)) {
            source[sourceName] = parentSource[sourceName] as ITypeSource;
        } else if (
...
function mergeTypes(
    ...
    return merge(type, parentType, specificProps) as IType | ITypeDescription;
}
...
function getTypeMetadata(type: ITypeDescription, name: string): unknown {
    ...
    if (meta.includes(name)) {
        return type[name as keyof ITypeDescription];
    }
...
interface ITypeStore {
    ...
    get: (typeId: TTypeId) => IType | undefined | null;
...
class TypeStore implements ITypeStore {
    ...
    get(typeId: TTypeId): IType | undefined {
...
export class TypeDescriptor {
     ...
     * @param typeId Идентификатор типа, информацию о котором нужно получить.
     */
     ...
     */
    getSource(typeId: TTypeId, mode: string): string | undefined {
     ...
     * @param mode Режим отображения.
     */
    getSourceArguments(typeId: TTypeId, mode: string): ISourceArguments | undefined {
        const source = this._getType(typeId).source;
     ...
     */
    getSourceConfigGetter(typeId: TTypeId, mode: string): string | undefined {
        const source = this._getType(typeId).source;
    ...
    getPropertiesArray(typeId: TTypeId): IPropertyArrayItem<unknown>[] {
        ...
        for (const key in properties) {
            // Object.assign сохраняет типы, а при spread они потеряются
            // eslint-disable-next-line prefer-object-spread
            propertiesArray.push(Object.assign({ name: key }, properties[key]));
        }
     ...
     * @param typeId Идентификатор типа, информацию о котором нужно получить.
     */
    getPermissions(typeId: TTypeId): string[] | undefined {
     ...
     * @param typeId Идентификатор типа, информацию о котором нужно получить.
     */
    getPermissionsMode(typeId: TTypeId): number | undefined {
     ...
     */
    getPropertyType(typeId: TTypeId, propertyName: string): string | undefined {
    ...
    getPropertyMetadata(typeId: TTypeId, propertyName: string, metadataName: string): unknown {
        ...
        if (!property) {
            throw new Error(`Свойство с именем ${propertyName} не существует`);
        }
        if (!property.propertyDescription) {
            throw new Error(`Свойство ${property} должно содержать поле propertyDescription.`);
        }
...
function getTDStore(key: string): IStore<Record<string, IType>> {
    if (!getStore(key)) {
        // TODO: формально тут ошибка, потому что this вообще window получится. Но трогать страшно
        // @ts-ignore
...
class SingletonTypeStore implements ITypeStore {
    ...
    has(typeId: TTypeId): boolean {
        ...
        // TODO: нет смысла править типы под костыль
        // @ts-ignore
...
class TypeDescriptorSingleton extends TypeDescriptor implements ISerializableState {
    ...
    getState(): ITypes {
        ...
        for (const key of Object.keys(store.toObject().__data)) {
            state[key] = store.get(key) as IType;
        }
        return state;
    }
