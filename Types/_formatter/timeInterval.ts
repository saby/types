...
function detectedMonths(config: IConfig) {
    let result = config.displayedUnits.years
        ? 0
        : detectedYears(config) * COUNT_MONTH_IN_YEAR;
    const start = config.startDate;
    const finish = config.finishDate;
    ...
    let months;
    if (finish.getFullYear() === start.getFullYear()) {
...
function buildStr(timeUnits: ITimeUnits) {
    ...
    if (units.length === 1) {
        return `0:${units[0] > TWO_DIGIT ? units[0] : `0${units[0]}`}`;
    }
...
function buildConfig(configuration: IConfig) {
    if (
        !(
            typeof configuration.time === 'number' ||
            configuration.startDate instanceof Date
        )
    ) {
        throw new TypeError(
            'Type is incorrect for field "time" or "startDate"'
        );
    }
    ...
    if (!config.startDate) {
        ...
        config.finishDate = config.finishDate
            ? new Date(config.finishDate.getTime())
            : new Date();
