...
import strategies from './date/strategies';
import getTzDate from './date/getTzDate';

import {
    DateFormattingStrategyType,
    IDateFormatConfig,
    DateFormattingStrategy,
} from './date/IDateFormat';
...
interface IInnerDateFormatConfig extends IDateFormatConfig {
    date: Date;
    mask: string;
}
...
const tokens: Record<string, [string | Function, IDateFormatOptions]> = {};
...
function withLeadZeroes(value: number, count: number): string {
    ...
    const sign = value >= 0 ? '' : '-';
    return sign + (String(Math.pow(DECIMAL_BASE, Math.max(0, restCount))).substr(1) + absValue);
}
...
function getQuarterNumber(date: Date): string {
    return getConfig().calendarEntities.quarters[getQuarter(date) - 1];
}

/**
 * Returns quarter number in minimized notation
 */
function getQuarterMin(date: Date): string {
    return getConfig().calendarEntities.minQuarters[getQuarter(date) - 1];
}

/**
 * Returns quarter number in short notation
 */
function getQuarterShort(date: Date): string {
    return getConfig().calendarEntities.shortQuarters[getQuarter(date) - 1];
}

/**
 * Returns quarter number in long notation
 */
function getQuarterLong(date: Date): string {
    return getConfig().calendarEntities.longQuarters[getQuarter(date) - 1];
}
...
/**
 * Returns localized half a year in minimized notation
 */
function getHalfYearMin(date: Date): string {
    return getConfig().calendarEntities.shortHalfYears[getHalfYear(date) - 1];
}

/**
 * Returns localized half a year in long notation
 */
function getHalfYearLong(date: Date): string {
    return getConfig().calendarEntities.longHalfYears[getHalfYear(date) - 1];
}
...
function formatByToken(
    ...
    if (options.lower && typeof result === 'string' && controller.currentLang !== 'en') {
...
addToken('YYhr', getHalfYearMin);
addToken('YYYY', 'getFullYear');
addToken('YYYYhr', getHalfYearLong);
addToken('Q', getQuarter);
addToken('Qr', getQuarterNumber);
addToken('QQr', getQuarterMin);
addToken('QQQr', getQuarterShort);
addToken('QQQQr', getQuarterLong);

// Time zone tokens
addToken('Z', getTimeZone, { separator: ':' });
addToken('ZZ', getTimeZone, { separator: '' });

function isValidMask(mask: unknown): boolean {
    return mask !== undefined && mask !== null && typeof mask === 'string';
}
...
function buildConfig(
    ...
    if (!isValidMask(result.mask)) {
        result.mask = getStrategy(result.strategy)(result.date, result);
    }

    return result as IInnerDateFormatConfig;
}
...
function getStrategy(strategy?: string | DateFormattingStrategy) {
    ...
    if (typeof strategy === 'string') {
        return (
            strategies[strategy as keyof typeof DateFormattingStrategyType] ||
...
/**
 * Чистая функция форматирования
 * @param date Дата.
 * @param configuration Конфигурация для форматирования даты.
...
/**
 * Доступные константы для вывода дат
 * @public
 */
export class Constants {
    /**
     * Цифровой формат месяца с полным годом "MM.YYYY" для "Ru-ru"
     */
    get DIGITAL_MONTH_FULL_YEAR(): string {
        return getConfig().date.DIGITAL_MONTH_FULL_YEAR;
    }
    /**
     * Продолжительность в кратком формате времени:, "HHH:mm" для "Ru-ru"
     */
    get DURATION_SHORT_TIME(): string {
        return getConfig().date.DURATION_SHORT_TIME;
    }
    /**
     * Продолжительность в полном формате времени:, "HHH:mm:SS" для "Ru-ru"
     */
    get DURATION_FULL_TIME(): string {
        return getConfig().date.DURATION_FULL_TIME;
    }
    /**
     * Полная дата с днем недели, "D MMMMlgen'YY, ddddl" для "Ru-ru"
     */
    get FULL_DATE_DOW(): string {
        return getConfig().date.FULL_DATE_DOW;
    }
    /**
     * Вывод дня, "D" для Ru-ru
     */
    get DAY(): string {
        return 'D';
    }
    /**
     * Полная дата с полным названием месяца, "D MM'YY" для "Ru-ru"
     */
    get FULL_DATE(): string {
        return getConfig().date.FULL_DATE;
    }
    /**
     * Полная дата с полным названием месяца, "D MMMMlgen'YY" для "Ru-ru"
     */
    get FULL_DATE_FULL_MONTH(): string {
        return getConfig().date.FULL_DATE_FULL_MONTH;
    }
    /**
     * Полная дата с полным названием месяца и полным годом, "D MMMMlgen YYYY" для "Ru-ru"
     */
    get FULL_DATE_FULL_MONTH_FULL_YEAR(): string {
        return getConfig().date.FULL_DATE_FULL_MONTH_FULL_YEAR;
    }
    /**
     * Полная дата с полным годом, "DD.MM.YYYY" для "Ru-ru"
     */
    get FULL_DATE_FULL_YEAR(): string {
        return getConfig().date.FULL_DATE_FULL_YEAR;
    }
    /**
     * Полная дата с кратким названием месяца, "D MMMlgen'YY" для "Ru-ru"
     */
    get FULL_DATE_SHORT_MONTH(): string {
        return getConfig().date.FULL_DATE_SHORT_MONTH;
    }
    /**
     * Полная дата с кратким названием месяца и полным годом, "D MMMlgen YYYY" для "Ru-ru"
     */
    get FULL_DATE_SHORT_MONTH_FULL_YEAR(): string {
        return getConfig().date.FULL_DATE_SHORT_MONTH_FULL_YEAR;
    }
    /**
     * Комбинированный формат полной даты и краткого времени:, "DD.MM.YY HH:mm" для "Ru-ru"
     */
    get FULL_DATE_SHORT_TIME(): string {
        return getConfig().date.FULL_DATE_SHORT_TIME;
    }
    /**
     * Комбинированный формат полной даты и полного времени:, "DD.MM.YY HH:mm:ss" для "Ru-ru"
     */
    get FULL_DATE_FULL_TIME(): string {
        return getConfig().date.FULL_DATE_FULL_TIME;
    }
    /**
     * Комбинированный формат полной даты и полного времени с миллисекундами:, "DD.MM.YY HH:mm:ss.SSS" для "Ru-ru"
     */
    get FULL_DATE_FULL_TIME_FRACTION(): string {
        return getConfig().date.FULL_DATE_FULL_TIME_FRACTION;
    }
    /**
     * Комбинированный формат полной даты с полным годом и краткого времени:, "DD.MM.YYYY HH:mm" для "Ru-ru"
     */
    get FULL_DATE_FULL_YEAR_SHORT_TIME(): string {
        return getConfig().date.FULL_DATE_FULL_YEAR_SHORT_TIME;
    }
    /**
     * Комбинированный формат полной даты с полным годом и полного времени:, "DD.MM.YYYY HH:mm:ss" для "Ru-ru"
     */
    get FULL_DATE_FULL_YEAR_FULL_TIME(): string {
        return getConfig().date.FULL_DATE_FULL_YEAR_FULL_TIME;
    }
    /**
     * Комбинированный формат полной даты с полным годом и полного времени с миллисекундами:, "DD.MM.YYYY HH:mm:ss.SSS" для "Ru-ru"
     */
    get FULL_DATE_FULL_YEAR_FULL_TIME_FRACTION(): string {
        return getConfig().date.FULL_DATE_FULL_YEAR_FULL_TIME_FRACTION;
    }
    /**
     * Полный формат даты и времени, "D MMMgen'YY HH:mm" для "Ru-ru"
     */
    get FULL_DATETIME(): string {
        return getConfig().date.FULL_DATETIME;
    }
    /**
     * Полное полугодие и год, "YYYYhr 'YY" для "Ru-ru"
     */
    get FULL_HALF_YEAR(): string {
        return getConfig().date.FULL_HALF_YEAR;
    }
    /**
     * Только полугодие, "YYYYhr" для "Ru-ru"
     */
    get ONLY_HALF_YEAR(): string {
        return 'YYYYhr';
    }
    /**
     * Только краткое полугодие, "YYhr" для "Ru-ru"
     */
    get ONLY_SHORT_HALF_YEAR(): string {
        return 'YYhr';
    }
    /**
     * Полное название месяца и год, "MMMM'YY" для "Ru-ru"
     */
    get FULL_MONTH(): string {
        return getConfig().date.FULL_MONTH;
    }
    /**
     * Полное название месяца, "MMMM" для "Ru-ru"
     */
    get MONTH(): string {
        return 'MMMM';
    }
    /**
     * Краткое название месяца, "MMM" для "Ru-ru"
     */
    ...
    /**
     * Полный квартал и год, ""QQQQr'YY"," для "Ru-ru"
     */
    get FULL_QUARTER(): string {
        return getConfig().date.FULL_QUARTER;
    }
    /**
     * Краткий квартал, "Qr" для "Ru-ru"
     */
    get QUARTER(): string {
        return 'Qr';
    }
    /**
     * Полный квартал, "QQQQr" для "Ru-ru"
     */
    get ONLY_QUARTER(): string {
        return 'QQQQr';
    }
    /**
     * Краткий квартал, "QQQr" для "Ru-ru"
     */
    get ONLY_SHORT_QUARTER(): string {
        return 'QQQr';
    }
    /**
     * Полный год, "YYYY" для "Ru-ru"
     */
    get FULL_YEAR(): string {
        return 'YYYY';
    }
    /**
     * Полное время, "HH:mm:ss" для "Ru-ru"
     */
    get FULL_TIME(): string {
        return getConfig().date.FULL_TIME;
    }
    /**
     * Полное время с миллисекундами, "HH:mm:ss.SSS" для "Ru-ru"
     */
    get FULL_TIME_FRACTION(): string {
        return getConfig().date.FULL_TIME_FRACTION;
    }
    /**
     * Дата и время в формате ISO
     */
    get ISO_DATETIME(): string {
        return getConfig().date.ISO_DATETIME;
    }
    /**
     * Дата и время в формате даты SQL
     */
    get ISO_DATETIME_SQL(): string {
        return getConfig().date.ISO_DATETIME_SQL;
    }
    /**
     * Краткая дата с днем недели, "D MMMMlgen, ddddl" для "Ru-ru"
     */
    get SHORT_DATE_DOW(): string {
        return getConfig().date.SHORT_DATE_DOW;
    }
    /**
     * Краткая дата с днем недели, "D MMMMlo, ddl" для Ru-ru
     */
    get SHORT_DATE_SHORT_DOW(): string {
        return getConfig().date.SHORT_DATE_SHORT_DOW;
    }
    /**
     * Краткая дата и краткий месяц с днем недели, "D MMMlo, ddl" для Ru-ru
     */
    get SHORT_DATE_SHORT_MONTH_SHORT_DOW(): string {
        return getConfig().date.SHORT_DATE_SHORT_MONTH_SHORT_DOW;
    }
    /**
     * Краткая дата, "DD.MM" для "Ru-ru"
     */
    get SHORT_DATE(): string {
        return getConfig().date.SHORT_DATE;
    }
    /**
     * Краткая дата с полным названием месяца, "D MMMMlgen" для "Ru-ru"
     */
    get SHORT_DATE_FULL_MONTH(): string {
        return getConfig().date.SHORT_DATE_FULL_MONTH;
    }
    /**
     * Краткая дата с кратким названием месяца, "D MMMlgen" для "Ru-ru"
     */
    get SHORT_DATE_SHORT_MONTH(): string {
        return getConfig().date.SHORT_DATE_SHORT_MONTH;
    }
    /**
     * Комбинированный формат краткой даты и краткого времени:, "DD.MM HH:mm" для "Ru-ru"
     */
    get SHORT_DATE_SHORT_TIME(): string {
        return getConfig().date.SHORT_DATE_SHORT_TIME;
    }
    /**
     * Комбинированный формат краткой даты и полного времени:, "DD.MM HH:mm:ss" для "Ru-ru"
     */
    get SHORT_DATE_FULL_TIME(): string {
        return getConfig().date.SHORT_DATE_FULL_TIME;
    }
    /**
     * Комбинированный формат краткой даты и полного времени с миллисекундами:, "DD.MM HH:mm:ss.SSS" для "Ru-ru"
     */
    get SHORT_DATE_FULL_TIME_FRACTION(): string {
        return getConfig().date.SHORT_DATE_FULL_TIME_FRACTION;
    }
    /**
     * Краткий формат даты и времени, "D MMMl HH:mm" для "Ru-ru"
     */
    get SHORT_DATETIME(): string {
        return getConfig().date.SHORT_DATETIME;
    }
    /**
     * Краткое полугодие, "YYhr 'YY" для "Ru-ru"
     */
    get SHORT_HALF_YEAR(): string {
        return getConfig().date.SHORT_HALF_YEAR;
    }
    /**
     * Краткое название месяца и год, "MMM'YY" для "Ru-ru"
     */
    get SHORT_MONTH(): string {
        return getConfig().date.SHORT_MONTH;
    }
    /**
     * Краткий квартал и год, "QQr 'YY" для "Ru-ru"
     */
    get SHORT_QUARTER(): string {
        return getConfig().date.SHORT_QUARTER;
    }
    /**
     * Краткое время, "HH:mm" для "Ru-ru"
     */
    get SHORT_TIME(): string {
        return getConfig().date.SHORT_TIME;
    }
}
 ...
 *
 */
export type FormatName = keyof Constants;

/**
 *
 */
export type IFormat = typeof format & Record<FormatName, string>;

/**
 * Преобразует дату в строку указанного формата. Содержит в себе костанты.
 * @remark
 * <h2>Стратегии форматирования дат</h2>
 * Стратегии форматирования дат - готовое решение для отображения даты и времени событий, формат которых зависит от того, когда их наблюдает пользователь. Рекомендуется применять в реестрах приложений.
 * Предлагается два типа стратегий: Registry и OnlyDate.
 * <h3>Зависимость формата от даты наблюдения</h3>
 * Рассмотрим отличия в форматах дат между стратегиями Registry и OnlyDate.
 * <ul>
 *     <li>
 *         Событие произошло/произойдёт в текущий для пользователя день:
 *         <ul>
 *             <li>Registry: <b>формат HH:mm</b> (например, 15:46);</li>
 *             <li>OnlyDate: <b>формат "D MMMl"</b> (например, 25 окт).</li>
 *         </ul>
 *     </li>
 *     <li>Если событие произошло/произойдёт не позже 2 месяцев от текущей даты:
 *          <ul>
 *             <li>Registry: формат "D MMMl" (например, 25 окт);</li>
 *             <li>OnlyDate: формат "D MMMl" (например, 25 окт).</li>
 *         </ul>
 *     </li>
 *     <li>Если произошло в текущем году, но не раньше 6 месяцев от текущей даты:
 *         <ul>
 *             <li>Registry: формат "D MMMl" (например, 25 окт);</li>
 *             <li>OnlyDate: формат "D MMMl" (например, 25 окт).</li>
 *         </ul>
 *     </li>
 *     <li>Если произошло в прошлом году, но не раньше 4 месяцев от текущей даты:
 *          <ul>
 *             <li>Registry: формат "D MMMl" (например, 25 окт);</li>
 *             <li>OnlyDate: формат "D MMMl" (например, 25 окт).</li>
 *         </ul>
 *     </li>
 *     <li>Если произошло в периоде, не подпадающем в указанные выше интервалы:
 *         <ul>
 *             <li>Registry: формат "DD.MM.YY" (например, 30.01.25);</li>
 *             <li>OnlyDate: формат "DD.MM.YY" (например, 30.01.25).</li>
 *         </ul>
 *     </li>
 * </ul>
 *
 * <h3>Форматирование даты с использованием стратегий</h3>
 * Пример иcпользования стратегии Registry:
 * <pre>
 *     import { DateFormattingStrategyType, date as format } from 'Types/formatter';
 *
 *     const eventDate = new Date(2023, 3, 25, 11, 0, 0); // дата события
 *     const config = {
 *         currentDate: new Date(2023, 3, 4, 12, 0, 0), // дата, относительно которого вычисляется период
 *         strategy: DateFormattingStrategyType.Registry,
 *     };
 *
 *     format(eventDate, config); // 25 Apr
 * </pre>
 * Пример использования стратегии OnlyDate:
 * <pre>
 *     import { DateFormattingStrategyType, date as format } from 'Types/formatter';
 *
 *     const eventDate = new Date(2022, 7, 5, 11, 0, 0); // дата события
 *     const config = {
 *         currentDate: new Date(2023, 3, 4, 12, 0, 0), // дата, относительно которого вычисляется период
 ...
 * </pre>
 *
 * <h2>Доступные константы для вывода дат {@link https://axure.tensor.ru/StandardsV8/%D1%84%D0%BE%D1%80%D0%BC%D0%B0%D1%82%D1%8B_%D0%B4%D0%B0%D1%82_%D0%B8_%D0%B2%D1%80%D0%B5%D0%BC%D0%B5%D0%BD%D0%B8.html по стандарту} с учетом локализации.</h2>
 * <ul>
 ...
 * </ul>
 *
 * <h2>Доступные константы для вывода дат {@link https://axure.tensor.ru/StandardsV8/поля_ввода_и_метки.html по стандарту полей ввода} с учетом локализации.</h2>
 * <ul>
 ...
 *     <li>MMMgen: сокращенное название месяца в текущей локали в родительном падеже (например, 'Янв' или 'Jan', 'Мая' или 'May');</li>
 ...
 *     <li>YYhr: номер полугодия и полугодие в текущей локали в краткой форме (например, 'I пл' или 'I hy');</li>
 *     <li>YYYYhr: номер полугодия и полугодие  в текущей локали в полной форме (например, 'I полугодие' или 'I half year');</li>
 *     <li>QQr: номер квартала и квартал в текущей локали в краткой форме (например, 'I кв' или 'I qt');</li>
 *     <li>QQQr: номер квартала и квартал в текущей локали в сокращенной форме (например, 'I квр' или 'I qtr');</li>
 *     <li>QQQQr: номер квартала и квартал в текущей локали в полной форме (например, 'I квартал' или 'I quarter').</li>
 ...
 * @public
 */
const dateFormat = format as IFormat;
