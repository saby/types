...
function getQuarterRoman(date: Date): string {
    // TODO Кость под английский и французский, там вместо римских должны использовать арабски числа для кварталов.
    //  По хорошему это должно регулироваться конфигом локали, но у нас даты пока привязаны к региону, а не языку.
    if (getConfig().code.startsWith('en') || getConfig().code.startsWith('fr')) {
...
function getQuarterRomanMin(date: Date): string {
    return getConfig().calendarEntities.minQuarter.replace('$digit$s$', getQuarterRoman(date));
}
...
function getQuarterRomanShort(date: Date): string {
    return getConfig().calendarEntities.shortQuarter.replace('$digit$s$', getQuarterRoman(date));
}
...
function getQuarterRomanLong(date: Date): string {
    return getConfig().calendarEntities.longQuarter.replace('$digit$s$', getQuarterRoman(date));
}
...
function getHalfYearRoman(date: Date): string {
    // TODO Кость под английский и французский, там вместо римских должны использовать арабски числа для полугодий.
    //  По хорошему это должно регулироваться конфигом локали, но у нас даты пока привязаны к региону, а не языку.
    if (getConfig().code.startsWith('en') || getConfig().code.startsWith('fr')) {
        return `${getHalfYear(date)}`;
    }
...
function getHalfYearRomanMin(date: Date): string {
    return getConfig().calendarEntities.minHalfYear.replace('$digit$s$', getHalfYearRoman(date));
}
...
function getHalfYearRomanLong(date: Date): string {
    return getConfig().calendarEntities.longHalfYear.replace('$digit$s$', getHalfYearRoman(date));
}
...
type IFormat = {
    [name in FormatName]: string;
} & ((date: Date, format?: string | IDateFormatConfig, timeZone?: number) => string);

export default format as IFormat;
