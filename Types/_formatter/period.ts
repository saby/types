 ...
 * @property {Types/formatter/PeriodConfigurationType.typedef} [configuration='Default'] Тип набора форматов для каждого типа периода. Можно передать свой набор.
...
function digitalPeriod(start: Date, finish: Date): string {
    ...
    return `${dateFormat(start, format)}${SEPARATOR}${dateFormat(
        finish,
        format
    )}`;
}
...
function datesPeriod(start: Date, finish: Date, type: Type): string {
    const format =
        type === Type.FullDate
            ...
            : dateFormat.FULL_DATE_SHORT_MONTH;
    ...
    const onlyDateFormat =
        type === Type.FullDate ? dateFormat.FULL_DATE_FULL_MONTH : 'D';
    ...
    if (startYear === finishYear) {
        ...
        if (startMonth === finishMonth) {
            ...
            return `${dateFormat(
                start,
                onlyDateFormat
            )}${SEPARATOR}${dateFormat(finish, format)}`;
        }
        return `${dateFormat(start, onlyMonthFormat)}${SEPARATOR}${dateFormat(
            finish,
            format
        )}`;
    }
...
function quartersPeriod(start: Date, finish: Date, type: Type): string {
    return generalPeriod(
        start,
        finish,
        type === Type.FullQuarter
            ? dateFormat.FULL_QUARTER
            : dateFormat.SHORT_QUARTER
    );
}
...
function halvesYearsPeriod(start: Date, finish: Date, type: Type): string {
    return generalPeriod(
        start,
        finish,
        type === Type.FullHalfYear
            ? dateFormat.FULL_HALF_YEAR
            : dateFormat.SHORT_HALF_YEAR
    );
}
...
function openPeriod(
    start: Date,
    finish: Date,
    formats: IPeriodFormats
): string {
    if (!start) {
        const format = formats.openStartPeriod.split(' ')[1];

        return formats.openStartPeriod.replace(
            format,
            dateFormat(finish, dateFormat[format])
        );
    }
    ...
    if (!finish) {
        ...
        return formats.openFinishPeriod.replace(
            format,
            dateFormat(start, dateFormat[format])
        );
    }
}
...
function build(
    ...
    if (
        ...
        (type === PeriodType.oneDay && options.useToday)
    ) {
    ...
    return `${dateFormat(
        start,
        dateFormat[startPeriod]
    )}${SEPARATOR}${dateFormat(finish, dateFormat[finisPeriod])}`;
}
