...
import Default from './period/Default';
import Accounting from './period/Accounting';
import WithoutYear from './period/WithoutYear';
import Text from './period/Text';
...
export interface IPeriodParams {
    ...
    /**
     * Использовать "Сегодня", если период это один день.
     */
    useToday?: boolean;
    /**
    ...
    short?: boolean;
    /**
     * Не отображать год, если период полностью входит в нынешний год. По умолчанию false.
     */
    hiddenYearIfCurrentYear?: boolean;

    currentDate?: Date;
}

interface IInnerPeriodParams {
    configuration?: ConfigurationType | IPeriodConfiguration;
...
const defaultOptions = {
    ...
    short: false,
    useToday: false,
    hiddenYearIfCurrentYear: false,
};

/**
 * Старый набор типов конфигураций.
 * @deprecated Надо использовать {@link IPeriodParams}
 */
export enum Type {
    /**
...
function isOpenPeriod(start: Date | null, finish: Date | null): boolean {
...
function buildQuartersOneYear(start: Date, finish: Date, formats: IPeriodFormats): string {
    const [startPeriod, finishPeriod] = formats[PeriodType.quartersOneYear];
    const disableYear = startPeriod.startsWith('ONLY') || finishPeriod.startsWith('ONLY');

    if (startPeriod !== 'QUARTER' && finishPeriod !== 'QUARTER') {
    ...
    const template = formats.quartersOneYearsTemplate;
    const startQrt = dateFormat(start, dateFormat.QUARTER);
    const finishQrt = dateFormat(finish, dateFormat.QUARTER);
    const year = disableYear ? '' : `'${dateFormat(finish, 'YY')}`;

    return `${template.replace(DIGIT_TOKEN, `${startQrt}${SEPARATOR}${finishQrt}`)}${year}`;
}

function getFormats(
    start: Date,
    finish: Date,
    { configuration, short, hiddenYearIfCurrentYear, currentDate }: IInnerPeriodParams
): IPeriodFormats {
    const formats =
    ...
    if (hiddenYearIfCurrentYear) {
        const currentYear = currentDate.getFullYear();

        if (currentYear === start.getFullYear() && currentYear === finish.getFullYear()) {
            return short ? formats.shortWithoutYear : formats.fullWithoutYear;
        }
    }
...
function build(start: Date | null, finish: Date | null, optionsUser: IPeriodParams): string {
    const options: IInnerPeriodParams = {
        ...defaultOptions,
        ...optionsUser,
        currentDate: optionsUser.currentDate || new Date(),
    };

    const formats = getFormats(
        start || options.currentDate,
        finish || options.currentDate,
        options
    );
    ...
    const [startPeriod, finishPeriod] =
        formats[type as Exclude<PeriodTypeName, PickType<IPeriodFormats, string>>];

    if (!finishPeriod) {
        return dateFormat(start, dateFormat[startPeriod]);
    }

    if (type === PeriodType.quartersOneYear) {
        return buildQuartersOneYear(start, finish, formats);
    }
