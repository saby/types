 ...
 */
function addCallback<T>(this: Promise<T>, onFulfilled: TCallback<T>): DeferredFromPromise<T> {
 ...
 */
function addErrback<T>(this: Promise<T>, onRejected: TErrBack<T>): DeferredFromPromise<T> {
...
function addCallbacks<T>(
    ...
    const def = new DeferredFromPromise<T>().addCallbacks(onFulfilled, onRejected);
    return bindCallbacks(this, def);
}
 ...
 * @param err результат асинхронной операции.
 * @param checkCallbackвключить проверку наличия callback обработки ошибок.
 */
...
function dependOn<T>(master: Promise<T> | Deferred<T>): Promise<T> {
    if (master instanceof Promise && !(master instanceof Deferred)) {
        return Promise.reject(new Error('Нельзя вызвать метод dependOn у Promise'));
    }
    let onFulfilled: (value: any) => void;
    let onReject: (error: Error) => void;

    const promise = new Promise((resolve: (error: any) => void, reject: (error: any) => void) => {
        onFulfilled = resolve;
        onReject = reject;
    });
 ...
 */
function createDependent<T>(this: Promise<T>): Promise<T> {
    return this.then(
        (value) => {
            return value;
        },
 ...
 * @return false - добавлять обработчики можно.
 */
...
function logWarn(message: string): void {
    import('Env/Env').then(({ IoC }) => {
        IoC.resolve('ILogger').warn('Types/PromiseAPIDeferred', message);
    });
}
 ...
 */
function skipLogExecutionTime(callback: (value: any) => any): (value: any) => any {
