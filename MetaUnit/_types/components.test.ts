...
describe('Meta/_types/components', () => {
    ...
    describe('ObjectComponentLoaderWithProps', () => {
        ...
        describe('constructor()', () => {
            ...
            it('не сериализует сохранённые `properties`', () => {
        ...
        describe('load()', () => {
            ...
            it('загружает только собственный редактор, если он задан', async () => {
                ...
                expect((result as any).getProperties().a?.getDesigntimeEditor()?.component).equal(
                    undefined
                );
                expect(
            ...
            it('загружает редакторы всех атрибутов, если собственный редактор не задан', async () => {
                ...
                expect(result.ready).equal(true);
                expect(result.component).equal(undefined);
                expect((result as any).getProperties().a?.getEditor()?.component).equal(Editor1);
                // expect(
