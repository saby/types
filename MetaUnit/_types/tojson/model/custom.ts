...
import { StringType, IPropertyEditorProps } from 'Meta/types';
import { Meta } from 'Meta/_types/baseMeta';
import { MetaClass, TMetaJson, TVariantMeta } from 'Meta/_types/marshalling/format';
...
export function getMetaWithRecordSet(items: [RecordSet, RecordSet]): Meta<string> {
    const result = StringType.id(id).editor<IEditorProps>('MetaUnit/_types/tojson/editor', {
        items,
        value: undefined,
        onChange: undefined,
    });
