...
describe('Meta/_types/marshaling', () => {
    describe('Meta', () => {
        ...
        it('Сериализуются sampleData', () => {
            ...
            // @ts-ignore
            assert.deepEqual(someMeta.toJSON()[0].sampleData, JSON.stringify({ data: sample }));

            const importPath = 'Foo/Bar';
            someMeta = someMeta.sampleDataImport(importPath);
            // @ts-ignore
            assert.deepEqual(someMeta.toJSON()[0].sampleData, JSON.stringify({ importPath }));
        });
        /**
    ...
    describe('WidgetMetaNew', () => {
        ...
        const jsonData = someMeta.toJSON(true);
